...
- **ARCHITECTURE RECONCILIATION (F1-7N-FA-3B-R0, 2026-08-20 — USER decision; supersedes the "KMFSR/KMOOP → sumRemainingShortages is the monthly producer" framing in the two notes below):** the canonical LIVE monthly engine is **Architecture A** — factory allocation `KMMSA`/`KMAR`, residual/net-order-need `KMTPP.projectTimePhasedSupply`, Suggested `calculateSuggestedOrderQty`, persist `47_`→`24_`→`KMPW` (see `SUPPLY_PLANNING_CALCULATION_RULES.md` §44). KMFSR = pure **§41 surplus-reallocation library** (invoked AFTER KMMSA/KMAR via a preallocated-input adapter, FA-3B1 — NOT the initial allocator); KMOOP = pure **§42 ongoing projection library** (monthly timing stays KMTPP-side, FA-3B2). `sumRemainingShortages` is a pure primitive, NOT the live monthly terminal owner. Both modules stay **IMPLEMENTED_NOT_PRODUCTION_CONNECTED**. Integration slices: FA-3B1 (KMFSR adapter) → FA-3B2 (KMOOP→KMTPP, timing-gated) → FA-3B3 (wire into Architecture A) → FA-3B4 (persistence/diagnostics).
- **Status note (F1-7N-FA-3B0, 2026-08-20):** the pure **Ongoing-Order site projection** landed — `assets/js/core/supply-planning-ongoing-order-projection.js` (`KMOOP.projectOngoingOrderSupply`), **PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED** (§42.4; 55 assertions). Projects `MAX(0, ordered − completed)` into per-site facts (A1 immutable request-source share; A2 within-company monthly `KMPCX.forecastShareQty` share; §43 FLOOR + residual; completed/shipped excluded; count-once). NOT wired to Monthly/Gap/Weekly/Request-Order; no KMFSR, no Net Order Need, no persistence.

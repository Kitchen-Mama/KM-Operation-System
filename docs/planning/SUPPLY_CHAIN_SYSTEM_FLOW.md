...
> - **Status:** Reviewed — B-1, B-2, B-3 Resolved (owners: Architecture Principles §8A.1; `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1 / §3.1B); B-4…B-8 Blockers Remain.
...
> - **Blocked By:** Batch B — Qualified Incoming allowlist · Request→PO atomicity · B-5/B-7/B-8 (see §11 Batch B Blockers below). **B-1 Reserve Trigger and B-2 Shipping Group Key no longer block — resolved (decision only; implementation not started).**

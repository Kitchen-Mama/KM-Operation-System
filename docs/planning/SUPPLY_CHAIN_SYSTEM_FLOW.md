...
> - **Status:** Reviewed — B-1, B-2, B-3 Resolved (decision only; owners: Architecture Principles §8A.1; `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1 / §3.1B); **B-4 CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED (2026-08-01, Round 4D-C — authority / external-origin quarantine / planning-admission / human-review / notification / no-automatic-external-fallback / count-once / dedup / ETA / source-identity contract resolved; Runtime / receiving / destination-identity / double-count / quarantine-review runtime remain open)**; **B-7 RESOLVED (2026-08-02, decision only — recommendation-cycle Composite Natural Key / Submit commitment boundary; Runtime not implemented)**; **B-5 RESOLVED (2026-08-03, decision only — `request_order_lines` / `request_order_line_sources` grain + quantity authority + Monthly SKU split + Recommendation→Request writer boundary; owner `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9; writer / DB migration / Runtime NOT implemented)**; B-6 / B-8 Blockers Remain. **Phase 2B Calculation Pure Runtime = FUNCTIONALLY COMPLETE / TEST VERIFIED (2026-08-03, Round 11A closure): all currently frozen pure contracts implemented — `SUPPLY_PLANNING_CALCULATION_RULES.md` §22/§27A/§32A/§34A + §39 Demand/Supply Ledger + §40 Overseas/Factory Allocation, plus the B-4 Minimal Pure Runtime chain (candidate → adapters → §2E Qualified Incoming → Line Runtime); §33 Golden Matrix = 39 executed / 1 pending / 0 canonical-blocked. The single remaining Golden #34 (User partial-carton Order Qty) is a downstream Request-Order / PO / UI-state / persistence acceptance (§37) — NOT a Calculation Pure Runtime blocker. Production Integration (real DB/source acquisition, receiving-layer, persistence writer, scheduler/retry/resume, LockService, Submit-guard, deployment) remains NOT COMPLETE / NOT PRODUCTION VERIFIED. The whole Recommendation Runtime is NOT complete — Recommendation Persistence / Orchestration Runtime remains pending.** **Recommendation Persistence / Orchestration Contract = FROZEN (2026-08-03, Phase 2C Round 1A — decision only; business flow at §5.5, full contract at `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Orch / `SYSTEM_RUNTIME_ARCHITECTURE.md` §7C); implementation NOT STARTED; next = implementation, then B-6 / B-8.** **Persistence CORE = IMPLEMENTED / TEST VERIFIED (Round 1B, 96 tests); Production Persistence Adapter Contract = FROZEN (Round 1C, §5.5 production flow / RRIS §Persist-Adapter — repo model B, script-lock reality, optimistic-concurrency token, additive `user_edited` + run-journal); adapter NOT IMPLEMENTED.**
> - **Current Version:** v1.7 (Batch B Round 4D-C: landed **§12 External-Origin Operation Discovery → Quarantine → Resolution flow**; B-4 registry → **CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED**; documentation only). v1.6 (Batch B Round 3 / B-3 Reissue: B-3 Marketplace Header/Line placement resolved in §11 registry — `shipping_plans.marketplace` = persisted derived scope marker (1→real / ≥2→`MULTI`); real Marketplace/Site SKU on lines; `shipment_line_plan_allocations` withdrawn; Plan→Shipment `0..1`; decision only). v1.5 (B-2 Shipping Group Key resolved — five-value Key). v1.4 (B-1 Reserve Trigger + `request_orders.request_status` residual fix).
> - **Last Reviewed:** 2026-08-03 (Phase 2C Round 1C — Production generate/persist flow FROZEN in §5.5 (readiness → calc outside lock → script lock → reload/revalidate → apply Plan → complete; failure/resume; decision only, owner RRIS §Persist-Adapter / ARCH §7C); adapter NOT IMPLEMENTED. Prior: Round 1A orchestration business flow; Batch B B-5 Decision Round; Round 11A Calc closure).
> - **Depends On:** Architecture Principles, Calculation Rules, Runtime Architecture, Database Relationship Map.
> - **Blocked By:** Batch B — **B-4 Qualified Incoming Runtime prerequisites** (receiving layer · `shipment_events` ETA · `destination_warehouse_id` persistence · `wh_on_the_way_qty` double-count; the **B-4 business + external-origin admission contract is RESOLVED (CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED)** — §11 B-4 / `SUPPLY_PLANNING_CALCULATION_RULES.md` §2E / §38) · Request→PO atomicity (B-6) · B-8 (see §11 Batch B Blockers below; **B-7 recommendation-cycle identity is RESOLVED — decision only; B-5 line/source grain + writer boundary is RESOLVED — decision only, §11 B-5 / RO&PO §3.9**). **B-1 Reserve Trigger and B-2 Shipping Group Key no longer block — resolved (decision only; implementation not started); the B-4 Qualified Incoming allowlist is no longer undecided — its business predicate + per-table direction are RESOLVED (B-4 CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED), only Runtime remains open.**
...
> **Shipment consolidation (canonical 2026-07-20; Group Key resolved 2026-07-31).** One Submit does **NOT** produce separate marketplace-specific plans — the **Shipping Group Key** excludes Marketplace (§11 B-2, resolved), so lines for different marketplaces on the same five-value route land in **one** plan whose header `marketplace` derives to `MULTI` (real marketplace kept on each line). Separately, at the Execution Layer, multiple approved plans sharing a compatible physical route/destination may be **consolidated by explicit human confirmation** ("Ready to Create") into **one physical `shipments` row**, linked via **`shipment_plan_links`** (many plans → one shipment; written only after the Shipment exists, unique `shipment_id + shipping_plan_id`, idempotent; **a plan transfers to at most one Shipment — Plan→Shipment `0..1`, no split**, B-3 2026-07-31). Per-plan/marketplace source stays traceable **on the original `shipping_plan_lines`, read back through `shipment_plan_links` as original planning context** (there is **no** Shipment-Line→Plan-Line ledger — `shipment_line_plan_allocations` is **WITHDRAWN**, B-3); PO/FIFO supply stays on the separate Supply-source axis `shipment_line_allocations`. See [`SHIPMENT_CENTER_SPEC.md`](./SHIPMENT_CENTER_SPEC.md) §2.A. Consolidation is **NOT IMPLEMENTED**.
...
Execution Plan Working Draft        (Persisted Recommendation Workspace — MAY persist to a non-commit DB Draft
        ...
        ↓  Submit Plan = Decision Commit   (reads ONLY the Execution Plan; group by the five-value Shipping Group Key [§11 B-2, resolved 2026-07-31 — Marketplace derived, not a key field]; cumulative into the same-Key open Draft; the ONLY creator of plans)
...
Weekly Shipping Plan Completed      (Decision Layer finished; completed_at set)
        ↓
Shipment Lifecycle                  (Execution Layer forward canonical DB statuses, owner `SHIPMENT_CENTER_SPEC.md` §3: Draft → [booking details recorded — no separate DB status] → Ready to Ship → Shipped → In Transit → Arrived → Received → Closed; `booked` is a non-status execution milestone, not a `shipments.status` token)
        ↓
...
3. **`request_order_line_sources`** — records per-source **demand-provenance** detail (`company` / `country` / `marketplace` / `site_sku` / `tier_type` (T1/T2/T3) / `source_month` (YYYY-MM) / `requested_qty` / `approved_qty` / snapshots / `note`). **Grain, natural key, writer contract, lineage, and lifecycle are RESOLVED under B-5 (§11 B-5 / `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9):** one `request_order_line` owns **1..N** sources; source natural key = `(request_order_line_id, country, marketplace, site_sku, source_month)` (+ additive `destination_warehouse_id` / `demand_key`); a source is **demand provenance, not a second quantity authority** (`Σ source.approved_qty = line.approved_qty`, line wins). The Company Allocation popup reads these rows and falls back to `request_order_lines` grouped by company (**"Site-level source pending"**) for legacy requests. The **full 1→N writer realization + lock/transaction is a later implementation round** (no code change in the decision). Canonical header status is **`request_orders.request_status`** (legacy `status` deprecated, read-fallback only); canonical bucket is **`request_order_lines.request_bucket`** (`tier_type` on lines deprecated). Deprecated `request_order_lines` columns (`final_order_qty`, `forecast_qty`, `current_stock`, `on_the_way_qty`, `factory_allocated_qty`, `reallocation_qty`, `source_company_count`, `source_site_count`, `product_name`, `need_reason`, `related_entity_type`, `related_entity_id`) are **no longer written or source of truth** (kept for back-compat, not deleted). Per-company allocation on lines is `km_qty` / `resus_qty` / `restw_qty`.
...
7. **Request Order Draft = Decision Layer; Purchase Order Overview = Execution Layer (finalized).** ALL ordering decisions — **Approved qty, company split (KM/ResUS/ResTW), T1 vs T2+T3, schedule dates, tier cancel** — are completed in **Request Order Draft**. **PO Overview inherits the approved result** and handles execution only (supplier / factory / payment / delivery dates); its **split/merge logic is PAUSED** and must not re-decide T1/T2/T3 until an explicit future design. Company split = **one `request_order_line` per company** (`company` column) is now **canonical** — line natural key `(request_order_id, company, sku, request_bucket)`, with per-site demand provenance in **1..N `request_order_line_sources`** (RESOLVED under B-5, §11 / RO&PO §3.9). **Factory display = `warehouses.warehouse_name`; `warehouse_id` stays the source of truth** (default Tier 1 = `WH-TW-CN-FACTORY-YOUXIN`).

> **Recommendation Persistence / Orchestration business flow (CONTRACT FROZEN — Decision Only, 2026-08-03; owner `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Orch / `SYSTEM_RUNTIME_ARCHITECTURE.md` §7C — NOT IMPLEMENTED).** The end-to-end recommendation flow is: **calculate (pure §39/§40) → resolve/refresh the single Active Draft (one per `recommendation_type + planning_cycle + business scope`, under a LockService lock) → user decision (edit `order_qty`/`planned_qty`; system `recommended_qty` immutable within a `draft_version`) → Submit (commitment boundary; never mutated by any later recommendation) → downstream handoff.** Handoff is a **distinct** command per type: Monthly **Send Request** copies eligible lines into `request_orders` via the B-5 §3.9 writer (company × sku × request_bucket lines + 1..N sources); Weekly **Create/Update Weekly Plan** promotes eligible `planned_qty` into `shipping_plans` (logistics layer). **Retry/Resume invariants:** a rerun reuses the existing Active Draft (never duplicates), reuses the same `calculation_run_id`, never overwrites user-edited quantities, and never returns empty-success after a partial write; `draft_version` increments only on a deliberate Manual Regenerate. **Neither the Draft nor Submit reserves stock** (reservation = the Shipment Ready-to-Ship transition, B-1). **Next dependency = IMPLEMENTATION** of this contract (§J phases), then **B-6** (Request→PO atomicity) and **B-8** (cancel/reopen/release) — both still open.
>
> **Production generate/persist flow (FROZEN — Decision Only, 2026-08-03 Round 1C; owner `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Adapter — NOT IMPLEMENTED).** The persistence core (Round 1B pure state machine, 96 tests) reaches Sheets through a repository adapter (model B): **verify readiness (§H) → run the pure calculation OUTSIDE the lock → acquire the project script lock (`getScriptLock`+`tryLock`; no native per-key lock in Apps Script) → reload the draft snapshot + revalidate the optimistic-concurrency token `{draft_version, userEditFingerprint}` → apply a deterministic Persistence Plan (diff) stage-by-stage (RUN_METADATA→HEADER→LINES→RECONCILE→LINEAGE→TOTALS→COMPLETED), marking each stage after its Sheet writes succeed → release the lock in `finally`.** **Failure/resume:** a partial run (`recommendation_calculation_runs.run_status = PARTIAL`) resumes from the stage after `current_stage`, reusing the same `calculation_run_id`; every stage is an idempotent natural-key upsert so replay never duplicates; **no success is reported until COMPLETED**; a failed run never destroys the previous good Active Draft; on a token mismatch the write does not overwrite (refresh retries; regenerate re-confirms). Requires additive `user_edited`/`user_edited_by` + a `recommendation_calculation_runs` table (not added here).
...
4. **`shipment_events`** — the **complete shipment lifecycle log**, e.g. **Created / Approved / Booked / Loaded / Departed / Arrived / Custom Clearance / Delivered / Received / Cancelled** *(these are `event_type` values — NOT `shipments.status` header tokens; `delivered` is an event, never a header status; `shipment_events` is spec-only / NOT IMPLEMENTED, `SHIPMENT_ROUTE_AND_EVENT_SPEC.md` §8/§9)*. Designed to **support future tracking integration** (carrier/API events append here).
...
> These two lifecycles are the **finalized** architecture for future implementation. **Runtime status = UNVERIFIED (this repair):** prior documentation recorded a possible implementation shape for `request_order_line_sources` (a DB tab with a header constant `13_procurement_handlers.gs:57`; a write at Create/Send Request; an `approved_qty` sync on Save · Submit · Convert to PO; a read by the adapter + Request Order Draft Company-Allocation UI). That observation was **not re-verified in this repair** and must not be treated as current or Canonical Runtime behaviour. **B-5 RESOLVED (§11 / RO&PO §3.9):** the source table's final grain, the **1→N** line→source mapping, its demand-provenance role (not a second quantity authority), and the Recommendation→Request writer identity are now canonical; the **full 1→N writer realization + lock/transaction is a later implementation round** (per-source cancel/release lifecycle = **B-8**, still open). The future **three-layer source design** and **`request_order_po_links`** grain fold into §3.9; **`shipment_events`** as a full lifecycle log remains spec-only. No schema/code change is made by this doc sync.
...
The following are the Batch B decisions. **B-1 is RESOLVED** (Batch B Round 1, 2026-07-30 — decision only; owner `SUPPLY_CHAIN_ARCHITECTURE_PRINCIPLES.md` §8A.1); **B-2 is RESOLVED** (Batch B Round 2, 2026-07-31 — decision only; owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1); **B-3 is RESOLVED** (Batch B Round 3 / Reissue, 2026-07-31 — decision only; owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1B); **B-4 is CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED (2026-08-01, Round 4D-C; the business + external-origin quarantine/admission contract is resolved — Runtime prerequisites remain open, see the B-4 registry row)**; **B-7 is RESOLVED** (Batch B B-7 Landing Round 5C, 2026-08-02 — decision only; recommendation-cycle identity = Composite Natural Key / Submit commitment boundary; owners `SYSTEM_RUNTIME_ARCHITECTURE.md` §7A / `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §G); **B-5 is RESOLVED** (Batch B B-5 Decision Round, 2026-08-03 — decision only; `request_order_lines` / `request_order_line_sources` grain, quantity authority, Monthly SKU split, and Recommendation→Request writer boundary; owner `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9; writer / DB migration / Runtime NOT implemented); **B-6 / B-8 remain NOT decided** and must not be guessed or pre-implemented. Undecided rows use the marker — **`BLOCKED — Requires Batch B Canonical Decision`** — and point here. Each row lists the conflict, the affected docs, and what is forbidden until the decision closes.
...
| **B-2** | **Shipping Group Key** — **RESOLVED (2026-07-31, decision only)** | **Resolved:** the Shipping Group Key = the **five values** `company + country + origin_endpoint (Ship From) + destination_endpoint (Destination) + shipping_method` (structured endpoint identity — group by canonical `warehouse_id` where present; `ship_from`/`destination` text = display snapshot only). **Marketplace is NOT a group-key field** — it is a **derived header scope**: 1 distinct effective-line marketplace → the actual Marketplace, ≥2 → **`MULTI`** (controlled scope marker, never a Marketplace FK, never used to look up marketplace master data); each `shipping_plan_lines` row keeps its **real** `marketplace` + `site_sku` (never merged in DB). `plan_month`/`plan_week` time dimensions stay on lines, never the Header key. **Draft-only cumulative Submit:** each Submit reuses the same-Key `status=draft` plan (update Header+Lines, recompute totals + Single/`MULTI`); if none, create a new Draft; an `approved`/any non-Draft same-Key plan is never modified → a new Draft is created. Same source-row retry is idempotent (no double count). The Key is **not** a permanent cross-history DB unique constraint; Runtime must avoid two writable Drafts per Key and flag (never random-pick) multiple legacy Drafts. **`parent_shipping_plan_id` = version-lineage anchor ONLY, never Combined-Plan membership** (a multi-marketplace "Combined Plan" is a single plan with derived `MULTI`). **Owner: `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1.** | this doc, WEEKLY_SHIPPING_PLAN_MAPPING_SPEC, DATABASE_RELATIONSHIP_MAP | Decision resolved; **Implementation / Runtime / Deployment Not Started; Runtime Verification Not Verified.** Still forbidden from this decision alone: adding a DB unique constraint on the Key; adding an unapproved line-level source-identity column; treating any grouping / header-derivation / `shipment_plan_links` Runtime as implemented. **`shipment_line_plan_allocations` is WITHDRAWN (B-3); cancellation/rollback stays B-8.** |
| **B-3** | **Marketplace Header／Line placement** — **RESOLVED (2026-07-31, decision only)** | **Resolved:** Marketplace is **NOT** a group-key column. `shipping_plans.marketplace` is a **persisted derived scope marker** recomputed from the effective `shipping_plan_lines` — **1 distinct** → the real Marketplace literal; **≥2 distinct** → **`MULTI`** (a non-FK scope marker; never a real Marketplace, never a Marketplace FK, never used to look up Site SKU / Rate Card / URL / Referral Rate / Marketplace Master Data). Not user-entered. Real Marketplace + Site SKU stay on the Lines as the physical columns `shipping_plan_lines.marketplace` / `shipping_plan_lines.site_sku` (never merged; no `marketplace_seperate`; same SKU may form multiple lines by Marketplace / Site SKU / period / source). Marketplace filtering must inspect the Lines, not only the Header. The 2026-07-27 amendment's contrary "no-MULTI" clauses are **partially superseded** (Option A precedence, 2026-07-31). **Owner: `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1B.** | this doc, WEEKLY_SHIPPING_PLAN_MAPPING_SPEC, DATABASE_RELATIONSHIP_MAP, SHIPPING_ALLOCATION_TO_SHIPMENT_CANONICAL_AMENDMENT_2026-07-27 | Decision resolved; **Runtime / Deployment Not Started; Not Verified.** Still forbidden: modeling Marketplace as a plan-group-key column; using `MULTI` as a real Marketplace / FK / master-lookup; adding an unapproved line source-identity column; creating `shipment_line_plan_allocations`. |
| **B-4** | **Qualified Incoming table-specific DB Status Allowlist** — **CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED (2026-08-01, Round 4D-C; supersedes the Round 4C-R2 partial-repair state)** | **Resolved (contract level; Runtime NOT implemented):** B-4 owns the **external-origin quarantine + planning-admission gate** (`SUPPLY_PLANNING_CALCULATION_RULES.md` §38; discovery→quarantine→human review→Link/Adopt/Reject/Ignore→admission flow §12; **no automatic external fallback** — a fresh **or** stale unlinked external record contributes **0** until an explicit human **Adopt** creates KM canonical lineage; linked external evidence never contributes separately; no fuzzy matching), and the **business qualification predicate** (ten-gate, `SUPPLY_PLANNING_CALCULATION_RULES.md` §2E), the **per-table status mapping** (Shipment allowlist `SHIPMENT_CENTER_SPEC.md` §10 = `ready_to_ship` / `shipped` / `in_transit` / `arrived`(delivered-not-received); excluded = `draft`/`received`/`closed`/`cancelled`/legacy `planned`·`completed`/`stuck`/event `delivered`; PO committed-supply `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §P1-B), the **ETA qualification** (ETA ≤ Required-By; interim authority = `shipments.eta`, long-term `shipment_events`, §2F), the **quantity authority** (canonical `shipment_lines.shipment_qty`; legacy `qty` fallback only), and the **count-once bucket-transition contract** (§30). Existing code still uses a terminal-state **denylist proxy** (`13_procurement_handlers.gs:407`) reading legacy `qty` — a known defect, correction IMPLEMENTATION_REQUIRED. **Still OPEN (Runtime not closed):** receiving-layer (received-qty / receipt handler / inventory posting), `shipment_events` ETA, `destination_warehouse_id` persistence, and the `wh_on_the_way_qty` ↔ Shipment-derived incoming double-count owner. **`request_order_line_sources` source/line grain = B-5; cancellation / unlock / reopen / rollback release = B-8 — NOT absorbed into B-4.** | this doc, Inventory Table Mapping, Calculation Rules (business semantics owner), Shipment Center, DB Map, Request Order/PO, Shipment Route & Event, Runtime | Still forbidden: claiming B-4 **RESOLVED / IMPLEMENTED / RUNTIME VERIFIED / COMPLETE**; treating raw On-the-way as Qualified Incoming without the predicate; using legacy `qty` as the canonical incoming quantity; declaring `warehouse_id` the permanent replacement for `destination_warehouse_id`; claiming `shipment_events` or the receiving layer is implemented; promoting Golden #12/#13/#14; treating a fresh or stale unlinked external record as automatically calculation-eligible (admission requires an explicit human **Adopt** to KM canonical lineage — §38 / §12); claiming the quarantine / notification / review / admission **Runtime** exists. **The next open prerequisite remains B-4's own Runtime prerequisites, not B-5 automatically.** |
| **B-5** | **`request_order_line_sources` final Grain／Writer／Lineage／Lifecycle** — **RESOLVED (2026-08-03, decision only)** | **Resolved:** line natural key = `(request_order_id, company, sku, request_bucket)`; **one line owns 1..N sources**, source natural key = `(request_order_line_id, country, marketplace, site_sku, source_month)` (+ additive `destination_warehouse_id` / `demand_key` lineage); a source is **demand provenance**, not a second quantity authority (`Σ source.approved_qty = line.approved_qty`, line wins); `approved_qty` is the single decision authority; `km/resus/restw` = transitional per-company snapshot (no 4th column); Monthly SKU split = by `company` × `request_bucket` (site dims → sources); Recommendation→Request writer input/output/idempotency identity frozen; `request_order_po_links` key = `(request_order_id, purchase_order_id)`. **Owner: `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9.** Writer / DB migration / Runtime NOT implemented. | this doc §5.5/§5.6, DB Map, RO&PO §3.9 | Decision resolved (decision only); **writer / persistence / lock / transaction / 1→N write realization NOT implemented.** Still forbidden: claiming the writer / DB migration / Runtime exists; deciding Request→PO atomicity (**B-6**) or source cancel/release lifecycle (**B-8**). |
...
| **B-7** | **Recommendation Cycle／Unique Key** — **RESOLVED (2026-08-02, decision only)** | **Resolved:** recommendation-cycle identity = a **Composite Natural Key** (Option B) — **no `recommendation_cycle_id` and no schema change now** (a future surrogate `recommendation_cycle_id` is permitted for **DB hardening only**, never replacing the natural-key business identity). **One Active Draft only** per `recommendation_type + planning_cycle + business scope` (Weekly key = `shipping_allocation_drafts` + `planning_cycle` (ISO `YYYY-Www`, Asia/Taipei) + company + country + marketplace + source_page + draft_version — destination is line-level; Monthly key = `request_order_allocation_drafts` + `planning_cycle` (`YYYY-MM`) + company + country + marketplace + draft_purpose + draft_version — Monthly `sku` split-grain **deferred to B-5**). **Scheduler / Retry / Resume reuse the existing Active Draft, never create a duplicate active Draft, and never overwrite user-edited `planned_qty` / `order_qty`.** **Manual Recommend / Regenerate** = an explicit rebuild with the latest data: the latest recommendation becomes the current Active Draft, `recommended_qty` is recalculated, `draft_version` increments; if manual quantities exist, **user confirmation is required before regenerating planning quantities**. **Submit = the business commitment boundary:** recommendation engines (manual or automatic) **must never mutate Submitted business records**; further change follows **Reject / Cancel / Reopen / New Revision** (release mechanics = B-8). **External OMS-created Inbound / Outbound records are never Recommendation authority** — they stay outside Recommendation until explicitly linked/adopted into KM Canonical Shipment per the B-4 authority contract. Recommendation is an operational assistant only; business execution authority begins only after Submit. Uniqueness is **Runtime-enforced** (deterministic lookup + project LockService + fail-closed); **no DB unique index is added here.** **Owners: `SYSTEM_RUNTIME_ARCHITECTURE.md` §7A · `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §G · `DATABASE_RELATIONSHIP_MAP.md`.** | Recommendation Runtime Spec, Runtime Architecture, DB Map | Decision resolved (decision only); **Runtime / composite-lookup / LockService / scheduler / writer / Regenerate / Submit-guard NOT implemented.** Still forbidden from this decision alone: adding a DB `cycle_key` column or unique index; treating composite-lookup / LockService / scheduler / writer / Regenerate / Submit-guard as implemented; deciding Reject/Cancel/Reopen release mechanics (**B-8**) or the Monthly `sku` split-grain (**B-5**). |
...
> These blockers are **spec-consolidation only** — no code, Apps Script, DB, or runtime is changed by recording them here. A Recommendation Draft may be persisted as a planning artifact; it is not Decision Truth, is not Qualified Incoming, does not reserve inventory, and does not create Inventory Movement. **B-1 is now resolved** (trigger = the successful Ready to Ship transition (`draft → ready_to_ship`) = Formal Shipment Execution Commit; decision only — owner §8A.1); excluding Draft was consistent with that resolution. **B-2 is now resolved** (five-value Shipping Group Key with Marketplace derived to a `MULTI` header scope; decision only — owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1). **B-3 is now resolved** (`shipping_plans.marketplace` = persisted derived scope marker 1→real / ≥2→`MULTI`; real Marketplace/Site SKU on `shipping_plan_lines`; `shipment_line_plan_allocations` withdrawn; decision only — owner §3.1B). **B-4 is now CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED (2026-08-01, Round 4D-C; predicate + per-table allowlist direction + ETA/quantity authority + count-once + external-origin quarantine/planning-admission gate aligned across the owner specs, §12 / `SUPPLY_PLANNING_CALCULATION_RULES.md` §38); Runtime / receiving-layer / `shipment_events` ETA / `destination_warehouse_id` persistence / double-count / quarantine-review-notification runtime remain IMPLEMENTATION_REQUIRED, and B-4 Runtime must NOT be treated as implemented.** **B-7 is now RESOLVED** (2026-08-02, decision only — recommendation-cycle identity = Composite Natural Key with one Active Draft per `recommendation_type + planning_cycle + business scope`; Scheduler/Retry/Resume reuse and never overwrite user quantity; **Submit is the business commitment boundary after which no recommendation engine may mutate a Submitted business record**; owners §7A / RRIS §G / DB Map; Runtime NOT implemented). **B-5 is now RESOLVED** (2026-08-03, decision only — `request_order_lines` key `(request_order_id, company, sku, request_bucket)`; one line owns 1..N demand-provenance sources; `approved_qty` single authority; Monthly SKU split by `company` × `request_bucket`; Recommendation→Request writer identity frozen; owner `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9; writer / DB migration / Runtime NOT implemented). B-6 / B-8 remain open.

---

## 12. External-Origin Operation Discovery → Quarantine → Resolution Flow (CANONICAL 2026-08-01 Round 4D-C — documentation only; Runtime NOT implemented)

An inbound or outbound operation may appear first in an OMS, WMS, warehouse platform, carrier, or Amazon platform **without** originating from the KM Operation System. KM may import and display such a record, but it **must never** enter Supply Planning, Qualified Incoming, committed supply, Current/Reserved Stock, Replenishment, or Shipping/Order Recommendation **automatically** — even when fresh, complete, and carrying a known warehouse/SKU/quantity/ETA/valid-status/stable-ID. **Freshness alone never authorizes planning admission.** (Business predicate + admission owner: `SUPPLY_PLANNING_CALCULATION_RULES.md` §2E / §38.)

**12.1 External-origin exception path (fail-closed):**
```
OMS/WMS external operation discovered
  → ingest source record (visible, auditable, contribution 0)
  → QUARANTINE (authority state = EXTERNAL_UNLINKED_QUARANTINED)
  → create/project EXCEPTION + notify responsible operational role
  → human review
  → LINK / ADOPT / REJECT / IGNORE_FOR_PLANNING / NEEDS_MORE_INFORMATION
  → KM canonical Shipment / Operation (only on Link or Adopt)
  → planning eligibility (admission — §38)
  → future calculation
```
Ingestion of unrelated valid records is **never** blocked by a quarantined record.

**12.2 Normal KM-origin path (unchanged):**
```
KM Formal Shipment → KM Inbound/Outbound Operation → push to OMS/WMS
  → external execution evidence returned → KM reconciliation → KM inventory movement
```
KM-origin candidates **bypass** external quarantine because KM canonical identity already exists.

**12.3 Human decision actions:** **Link** (external = existing KM Shipment/Operation → evidence only, KM owns Incoming, external qty contributes 0), **Adopt** (legitimate, no KM Shipment yet → explicit controlled creation/association of a KM canonical record; only the resulting KM quantity may calculate), **Reject** (duplicate / wrong company·warehouse·SKU / cancelled / test / invalid → auditable, 0, state `REJECTED_EXTERNAL_RECORD`), **Ignore-for-Planning** (valid externally but not KM planning-relevant → visible, 0, reason + actor required), **Needs-More-Information** (unclear → stays quarantined, 0, notification open). A generic "Approve" that bypasses Link/Adopt semantics is **not** allowed.

**12.4 Authority is fail-closed:** unknown or external-unlinked authority → visible + reviewable + planning contribution 0. **Linked external quantities never contribute separately; adopted records enter calculation only through the resulting KM canonical record (count-once).** No fuzzy matching (stable source identity only — `DATABASE_RELATIONSHIP_MAP.md`). The notification / exception queue / review / admission are a **future Runtime service — NOT implemented** here (no email, UI, webhook, or scheduler). The scheduler must **not** count review-pending external records; Daily Import does **not** auto-approve external records or auto-run Recommendation.

---

**v1.7 Architecture Specification. Documentation only; no code or DB changes are implied by this document. Calculation formulas are owned by the active `SUPPLY_PLANNING_CALCULATION_RULES.md` (the current canonical formula SSOT). B-1 Reserve Trigger resolved (§11 — decision only, owner §8A.1); B-2 Shipping Group Key resolved (§11 — decision only, owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1); B-3 Marketplace Header/Line placement resolved (§11 — decision only, owner §3.1B; `shipment_line_plan_allocations` withdrawn); **B-4 CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED (external-origin quarantine + planning-admission gate landed 2026-08-01 Round 4D-C, §12 / `SUPPLY_PLANNING_CALCULATION_RULES.md` §38; Runtime / receiving / destination-identity / double-count / quarantine-review runtime prerequisites open); Batch B blockers B-5 … B-8 (§11) remain open.**

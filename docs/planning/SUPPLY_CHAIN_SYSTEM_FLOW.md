...
> - **Status:** Reviewed — B-1, B-2, B-3 Resolved (decision only; owners: Architecture Principles §8A.1; `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1 / §3.1B); **B-4 PARTIALLY RESOLVED — business contract repaired (2026-08-01, Round 4C-R2); Runtime / receiving / destination-identity / double-count prerequisites remain open**; B-5 / B-6 / B-7 / B-8 Blockers Remain.
> - **Current Version:** v1.6 (Batch B Round 3 / B-3 Reissue: B-3 Marketplace Header/Line placement resolved in §11 registry — `shipping_plans.marketplace` = persisted derived scope marker (1→real / ≥2→`MULTI`); real Marketplace/Site SKU on lines; `shipment_line_plan_allocations` withdrawn; Plan→Shipment `0..1`; decision only). v1.5 (B-2 Shipping Group Key resolved — five-value Key). v1.4 (B-1 Reserve Trigger + `request_orders.request_status` residual fix).
...
The following are the Batch B decisions. **B-1 is RESOLVED** (Batch B Round 1, 2026-07-30 — decision only; owner `SUPPLY_CHAIN_ARCHITECTURE_PRINCIPLES.md` §8A.1); **B-2 is RESOLVED** (Batch B Round 2, 2026-07-31 — decision only; owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1); **B-3 is RESOLVED** (Batch B Round 3 / Reissue, 2026-07-31 — decision only; owner `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1B); **B-4 is PARTIALLY RESOLVED — business contract repaired (2026-08-01, Round 4C-R2; not fully decided — Runtime prerequisites open, see the B-4 registry row)**; **B-5 … B-8 remain NOT decided** and must not be guessed or pre-implemented. Undecided rows use the marker — **`BLOCKED — Requires Batch B Canonical Decision`** — and point here. Each row lists the conflict, the affected docs, and what is forbidden until the decision closes.

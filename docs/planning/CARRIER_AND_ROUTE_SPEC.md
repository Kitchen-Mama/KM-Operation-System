...
| `customs_type_label` | string | localized (中文) display Label for `customs_type`, **derived from the canonical enum→Label map** (`third_party_customs`=買單報關 / `formal_customs`=正式報關 / `tax_refund_customs`=退稅報關) at import (row override honored). Display metadata (like `shipping_method_label`); Master editable / Update locked. **(AMENDED 2026-07-28)** — this label is **NO LONGER snapshotted onto shipments/plans** (`shipments.shipments_customs_type_label` / `shipping_plans.customs_type_label` are RETIRED). It stays on `carrier_rate_cards` as display metadata + a resolver candidate source **but is NEVER a matching key** (matching uses the `customs_type` CODE). `shipments` persist only the CODE `shipments_customs_type`; `{{CUSTOMS_TYPE}}` resolves the Label from that code at render (Code→Display resolver). |
| `import_duty_treatment` | enum | **(2026-07-28)** `included_in_rate` / `excluded_in_rate` — whether import duty is already inside the carrier rate. **Blank = needs data completion** (a valid stored state). **NEVER auto-derived from `customs_type`** (買單/正式/退稅 must not set it) and a blank must **never** be treated as a known cross-border result. Carrier-editable (Update + Master import). Snapshotted onto `shipping_plans` / `shipments`; drives the Duty cost rule (included → Duty 0; excluded → series-based Duty; blank → Not Applied). |
...
> **IMPLEMENTED 2026-07-28 (unified Rate Matcher `17_carrier_handlers.gs` `shippingRateMatch_`):** the shared matcher provides three modes — **recommendation** (origin/dest country + battery; method/last_mile/customs are OUTPUT — Execution Plan / Weekly L1), **rough** (+ method + last_mile + customs — Weekly Plan; multiple candidates → user selects, never auto-cheapest/first), **exact** (+ carrier + city/postal/warehouse/marketplace — Shipment Draft). **All matching uses CODE / ID — never Label / Name.** `shipping_method_label` / `customs_type_label` are display metadata + resolver candidate sources, **never matching keys**; one CODE carrying inconsistent labels raises a **Data-Quality warning** on import (same method/customs, not different). **Carrier resolution:** import resolves `carrier_name` → a **unique** `carrier_id` (ambiguous/unknown name = rejected, never a Name FK); `carrier_name` is **never stored** on `shipping_plans` / `shipments` / `carrier_rate_cards` — resolved live from `carriers` at render.

...
| `supply-planning-surplus-reallocation.js` (RESERVED) | **RESERVED_NOT_IMPLEMENTED → FA-3A** — Factory Surplus Reallocation orchestrator (`KMFSR.projectSurplusReallocation`); wraps §35/§40 allocators + §32A primitives → §12 `sumRemainingShortages`; PLANNING-ONLY (no physical reservation/movement, §41). Once landed it becomes the missing producer that makes `net_order_need_snapshot` post-reallocation — supersedes the "no producer / FROZEN_NOT_IMPLEMENTED net_order_need" notes elsewhere in this doc. |

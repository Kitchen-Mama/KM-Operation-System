...
- **Everything downstream of a *decided* quantity is live-wired and reachable:** allocation-draft persist → `request_orders` / `request_order_lines` / `request_order_line_sources` (`13_procurement_handlers.gs:handleCreateRequestOrderDraft_`) → Decision-Layer edits (`request-order-draft.js`) → Convert-to-PO with T1 / T2_T3 split (`13_…:handleCreatePurchaseOrderFromRequest_(1317)`) → PO receive (`handleReceivePurchaseOrderLines_`). **FA-3B0-PRE:** PO receive now also performs the atomic count-once **Factory Stock handoff** — `completed_qty += recv` + `factory_stock.fac_current_stock += recv` + one `po_receipt` movement (shared `21_:factoryStockApplyDeltaTx_`), so the Ongoing-Order bucket is `MAX(0, ordered − completed)` and `completed − shipped` is physical Factory Stock (SUPPLY_PLANNING §42.3). Status = `FROZEN_AND_IMPLEMENTED` / `SOURCE_PRESENT_RUNTIME_UNVERIFIED`.
...
| `supply-planning-recommendation-orchestrator.js` / `-recommendation-source-integration.js` / `-plan-builder.js` | **KEEP_AS_CANONICAL** (pure glue; the composed reader→ledger→allocation→resolver→bridge chain already exists). **NOTE (F1-7N-FA-3A.0):** this "orchestrator" is composition GLUE — it is NOT the donor/surplus reallocation engine. The Factory Surplus Reallocation orchestrator is a SEPARATE reserved module `supply-planning-surplus-reallocation.js` (namespace **KMFSR**, §41 / F1-6, PLANNING-ONLY, NOT IMPLEMENTED until FA-3A). Do not conflate the two names. |
| `supply-planning-surplus-reallocation.js` | **IMPLEMENTED_NOT_PRODUCTION_CONNECTED (FA-3A)** — pure **§41 factory-surplus-reallocation LIBRARY** (`KMFSR`); §32A primitives + §41.5A timely-transfer authority; integer safety §43; 270 assertions. **CORRECTED (§44):** it is NOT the live monthly factory allocator (that is KMMSA/KMAR) and NOT the live `net_order_need` producer (that is KMTPP residual). `projectSurplusReallocation` runs its own §40 initial allocation = the STANDALONE model; live integration (FA-3B1/B3) uses a preallocated-input §41 adapter over KMMSA/KMAR coverage → adjusted `factoryCoveredQty` → KMTPP. `sumRemainingShortages` is a pure primitive, not the live monthly terminal. NOT wired to monthly/Gap/Weekly/Request-Order. |
| `supply-planning-ongoing-order-projection.js` | **PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED (FA-3B0)** — Ongoing-Order (unshipped PO) site projection (`KMOOP.projectOngoingOrderSupply`); §42 lifecycle partition (`notYetReceived = MAX(0, ordered − completed)`; completed→Factory Stock, shipped→In-Transit excluded); A1 = immutable `request_order_line_sources.requested_qty` share; A2 = within-company monthly Regular-FC share (`KMPCX.forecastShareQty` basis, assembly — no new formula); §43 FLOOR + residual; single-company; count-once. Does NOT call KMFSR / §40 / Net Order Need / carton rounding; no persistence. **§44:** pure library — monthly timing owner = KMTPP-side (not KMOOP); live integration (FA-3B2) maps output → KMTPP incoming, gated by `ONGOING_ORDER_MONTHLY_TIMING_AUTHORITY_MISSING`. Status IMPLEMENTED_NOT_PRODUCTION_CONNECTED. |

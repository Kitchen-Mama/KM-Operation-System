...
| `supply-planning-surplus-reallocation.js` | **PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED (FA-3A)** — Factory Surplus Reallocation orchestrator (`KMFSR.projectSurplusReallocation`); wraps §35/§40 `allocateFactoryDeterministic` + §32A primitives → §12 `sumRemainingShortages`; PLANNING-ONLY (no physical reservation/movement, §41); timely-transfer authority = §41.5A (CURRENT FACTORY STOCK tier gate; `timelyTransferableQty = donorRemainingReleasableSurplus`); integer safety per §43 (no ratio→unit conversion; §40 integer FIFO); 270 unit assertions. It is the missing producer that makes `net_order_need_snapshot` post-reallocation — supersedes the "no producer / FROZEN_NOT_IMPLEMENTED net_order_need" notes elsewhere. **NOT yet connected** to monthly / Gap / Weekly AI Plan / Request Order runtime (Phase-2 / FA-3B/FA-3H). |
| `supply-planning-ongoing-order-projection.js` | **PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED (FA-3B0)** — Ongoing-Order (unshipped PO) site projection (`KMOOP.projectOngoingOrderSupply`); §42 lifecycle partition (`notYetReceived = MAX(0, ordered − completed)`; completed→Factory Stock, shipped→In-Transit excluded); A1 = immutable `request_order_line_sources.requested_qty` share; A2 = within-company monthly Regular-FC share (`KMPCX.forecastShareQty` basis, assembly — no new formula); §43 FLOOR + residual; single-company; count-once. Does NOT call KMFSR / §40 / Net Order Need / carton rounding; no persistence. NOT wired to Monthly/Gap/Weekly/Request-Order (FA-3B). |

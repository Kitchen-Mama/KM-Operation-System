...
> - **Status:** Reviewed — B-1 Resolved (owner Architecture Principles §8A.1; no schema change); B-2…B-8 Blockers Remain.
> - **Current Version:** v1.5 (Batch B Round 1: B-1 reserve boundary resolved as the **Ready to Ship transition (`draft → ready_to_ship`) = Formal Shipment Execution Commit**; `request_orders.request_status` residual fix; Shipment Draft/Overview filter unified).
> - **Last Reviewed:** 2026-07-30.
> - **Depends On:** the domain schema specs it maps.
> - **Blocked By:** Batch B — Shipping Group Key · Qualified Incoming allowlist · `request_order_line_sources` remaining design (see `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11). **B-1 Reserve Trigger no longer blocks — resolved (decision only; no reserve write path is added — Runtime/Deployment Unverified).**

**Status:** 🟢 v1.5 — Database Relationship Specification (documentation only; Batch A canonical repair 2026-07-28; Batch B Round 1 B-1 resolution 2026-07-30)
...
The **canonical Batch B Decision Registry (fixed IDs B-1 … B-8) is owned by `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11.** This section only notes the **schema-affecting** subset, using those **same fixed IDs** — it does not renumber or re-title them. **B-1 is RESOLVED** (owner Architecture Principles §8A.1 — decision only, no schema change); undecided rows use the marker **`BLOCKED — Requires Batch B Canonical Decision`**.
...
| **B-2 Shipping Group Key** | Which fields form the plan group key. | Code groups by 5 route fields; no Batch A body asserts a canonical key. | Modeling any assumed key set; adding a unique constraint on an assumed key. |
| **B-3 Marketplace Header／Line placement** | Is Marketplace a group-key column or header-derived only. | Marketplace derived onto the header in code. | Modeling Marketplace as a plan-group key column. |
...
>   - **Group key — BLOCKED (B-2) + Marketplace placement BLOCKED (B-3):** the exact fields that form the Shipping Plan group key, and whether **Marketplace** is a key field or header-derived, are **NOT decided** (`SUPPLY_CHAIN_SYSTEM_FLOW.md` §11 B-2 / B-3). No schema, writer, unique constraint, payload, grouping or migration may depend on an assumed key. `shipping_plan_lines` keeps each line's real `marketplace` + `site_sku` (never merged in the DB); how `shipping_plans.marketplace` is populated depends on the B-3 decision and is not fixed here.
...
>   - The header plan-scope context (`company` / `country` / `marketplace` / `ship_from` / `destination` / `shipping_method`) + `carrier_id` + `shipment_total_qty` / `shipment_total_cartons` are **copied from the approved `shipping_plan` at Execution Commit (not recalculated)**. *(Which of these fields form the plan **group key** vs are header-derived is **BLOCKED — Requires Batch B Canonical Decision**, §7.6A B-2 — the copy itself is unaffected.)* Editable execution fields: carrier / booking / container / BL / invoice / ETD / ETA / tracking / dates / costs / `note` / `status`.
...
- `shipping_route_rules` provides routing **defaults only** — the Weekly Shipping Plan's chosen `ship_from` / `destination` win and are persisted on `shipping_plans`. *(Whether these are part of the plan **group key** is **BLOCKED — Requires Batch B Canonical Decision**, §7.6A B-2.)*

...
- **`document_template_fields`** = placeholder **mapping layer** (template token → data source / format / transform). Placeholders = UPPERCASE snake in `{{ }}`. **Population deferred.**

...
> - **Current Version:** v1.4 (Batch A repair: `request_order_line_sources` implemented-status correction + Batch B Schema Decisions pointer).
> - **Last Reviewed:** 2026-07-28.
> - **Depends On:** the domain schema specs it maps.
> - **Blocked By:** Batch B — Reserve Trigger · Shipping Group Key · Qualified Incoming allowlist · `request_order_line_sources` remaining design (see `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11).

**Status:** 🟢 v1.4 — Database Relationship Specification (documentation only; Batch A canonical repair 2026-07-28)
...
- **Company-based identity (Current Evidence):** in the current code path, lines are keyed by `request_order_id + company + sku + tier` (tier = `request_bucket` T1/T2/T3). The **final grain, writer, lineage and lifecycle of `request_order_line_sources` — including whether "one company = one line = one source" is the canonical rule — are Blocked under B-5** (`SUPPLY_CHAIN_SYSTEM_FLOW.md` §11); this line records observed behaviour, not a canonical grain decision.
...
> Finalized end-to-end chains (see `SUPPLY_CHAIN_SYSTEM_FLOW.md` §5.5 / §5.6). **Implementation-status correction (Batch A 2026-07-28, code-verified):** `request_order_line_sources` is **IMPLEMENTED** (real DB tab, header constant `13_procurement_handlers.gs:57`; written at Create/Send Request; `approved_qty` synced on Save · Submit · Convert to PO; read by adapter + Request Order Draft UI). Its **per-source status lifecycle** (status transition on cancel), the future **three-layer source design**, **`request_order_po_links`**, and the full **`shipment_events`** lifecycle log remain **NOT implemented**. No schema/code change is made by this documentation sync; existing table names unchanged. Undecided design points are consolidated in the **Batch B Schema Decisions Required** section (§7.6A) and `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11.

### 7.6A Batch B Schema Decisions Required (references the canonical registry)

The **canonical Batch B Decision Registry (fixed IDs B-1 … B-8) is owned by `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11.** This section only notes the **schema-affecting** subset, using those **same fixed IDs** — it does not renumber or re-title them. Marker: **`BLOCKED — Requires Batch B Canonical Decision`**.

| Fixed ID | Schema-affecting question | Current code-verified state | Forbidden until decided |
|---|---|---|---|
| **B-1 Exact Reserve Trigger** | Which event writes `fac_reserved_stock`, and is a reserved column even maintained? | No reserve write exists; only `fac_current_stock` deducted at Confirm Shipment & Dispatch. | Adding/relying on a reserve write path. |

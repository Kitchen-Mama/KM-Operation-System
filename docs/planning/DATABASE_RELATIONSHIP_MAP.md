...
>   - **Header (canonical):** `allocation_draft_id`, `planning_cycle`, `source_page`, `company`, `country`, `marketplace`, `status`, **`generation_type` (`scheduled`/`manual_refresh`/`user_created`)**, `calculation_run_id`, `calculated_at`, `source_data_as_of`, `draft_version`, audit (`created_*`/`updated_*`), `submitted_*`, `cancelled_*` + `cancel_reason`, `note`. **REMOVED:** `sku` (grain → lines), `target_window` (→ per-line), `source_type` (→ `generation_type`). **Uniqueness / cycle-key mechanism = BLOCKED BY B-7** (`SUPPLY_CHAIN_SYSTEM_FLOW.md` §11) — NOT decided: `planning_cycle` / `draft_version` are **CURRENT-OBSERVED header fields, not an approved uniqueness contract**; no composite key or unique index is ratified here.
...
| **B-7 Recommendation Cycle／Unique Key** | Whether a dedicated `cycle_key` column / composite key / unique index is used. | No Draft table persists a reliable cycle key. | Proposing any cycle-key column or unique index as decided. |

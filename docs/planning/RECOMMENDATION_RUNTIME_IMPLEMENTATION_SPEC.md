...
- **Layer 1 — Recommendation Allocation** (calc-owned, `SUPPLY_PLANNING_CALCULATION_RULES.md` §39/§40): total allocation ≤ calculated available supply; integer output; no fractional remainder; Phase-1 does **not** borrow stock (borrowing/reallocation = Phase 2).
...
Partial Submit; selectable lines; cross-company borrowing; allocation above the current pool; priority-based reallocation; full multi-ticket/multi-vessel execution split; Draft merge; submitted-Draft reopen; submitted-Recommendation revision mutation; advanced Cancel/Reopen/New-Revision contract; automatic stock borrowing. **All Phase 2 — not designed here.**

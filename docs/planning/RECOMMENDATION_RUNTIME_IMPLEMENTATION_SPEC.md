...
## External-Origin-Aware Implementation Order (CANONICAL 2026-08-01 Round 4D-C — implementation tracker; B4-R1–B4-R4 implemented + test-verified at source/pure-module level, B4-R5–B4-R8 not implemented, no deployment / live Runtime verified)
...
Refines the External-Origin-Aware Implementation Order (above) into the **smallest dependency-ordered, independently-testable batches** that can truthfully execute Golden #12 / #13 / #14. **B4-R1–B4-R4 are implemented and test-verified at source / pure-module level (Apps Script deployment / live Runtime UNVERIFIED); B4-R5–B4-R8 are not implemented.** Main is the sole canonical repository. No schema migration, no scheduler, no receiving, no external API, no notification, no ledger.
...
| **B4-R4** | KM Shipment Incoming Adapter — status allowlist, remaining qty, ETA, company/SKU/destination scope, exclusion reasons | adapter module + fixtures | adapter | NEW ADAPTER REQUIRED | **PURE ADAPTER IMPLEMENTED — UNIT/INTEGRATION FIXTURE VERIFIED** (2026-08-01; `assets/js/core/supply-planning-incoming-adapters.js` + `assets/tests/supply-planning-incoming-adapters.test.js`, 65 assertions) — Apps Script / Line Runtime integration **UNVERIFIED** |

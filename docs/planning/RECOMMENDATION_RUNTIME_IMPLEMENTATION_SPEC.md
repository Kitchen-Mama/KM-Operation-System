...
> - **Blocked By:** Batch B — **B-4 Runtime implementation prerequisites** (described in §B4-Plan) · **B-7** persisted recommendation-cycle / unique-key design · B-5 / B-6 / B-8 where applicable. **B-1 / B-2 / B-3 are RESOLVED (decision only); the B-4 contract is RESOLVED and B-4 Runtime implementation is IN PROGRESS at source level (B4-R1 + B4-R2 are SOURCE IMPLEMENTED — TEST VERIFIED; Apps Script deployment / live runtime UNVERIFIED); there is no "next open decision = B-2". The current authorized implementation task is B4-R3 (§B4-Plan).**

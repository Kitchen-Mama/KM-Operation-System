...
> + `net_order_need_snapshot` (LIVE monthly = `KMTPP.projectTimePhasedSupply` residual, SUPPLY_PLANNING §44; `sumRemainingShortages` is a pure primitive only) — persisted draft-line snapshots that stay **blank
...
- **Layer 1 — Recommendation Allocation** (calc-owned, `SUPPLY_PLANNING_CALCULATION_RULES.md` §39/§40): total allocation ≤ calculated available supply; integer output; no fractional remainder; Phase-1 does **not** borrow or move **physical** stock. **Disambiguation (F1-7N-FA-3A.0):** "borrowing/reallocation = Phase 2" refers to **PHYSICAL reservation / borrowing / inventory movement / ownership transfer** only. **ANALYSIS-ONLY / PLANNING-ONLY surplus netting** for recommendation math (§32/§32A/§41) **IS Phase-1 AUTHORIZED** — it changes `net_order_need_snapshot` / `reallocation_in/out_qty_snapshot` / Suggested Qty only and mutates no physical pool. **CORRECTED (F1-7N-FA-3B-R0; SUPPLY_PLANNING §44):** in the LIVE monthly engine (Architecture A) factory allocation = KMMSA/KMAR and the residual/Net-Order-Need owner = `KMTPP.projectTimePhasedSupply`; §41/`KMFSR` is a pure surplus-reallocation library (`IMPLEMENTED_NOT_PRODUCTION_CONNECTED`) that, once integrated (FA-3B1/B3), adjusts *factory coverage* feeding KMTPP — it is NOT itself the live net-order-need producer. See §44 and DECISION_REGISTER PART B (resolved).
...
Partial Submit; selectable lines; cross-company **physical** borrowing; allocation above the current pool; **physical** priority-based reallocation (reservation/movement); full multi-ticket/multi-vessel execution split; Draft merge; submitted-Draft reopen; submitted-Recommendation revision mutation; advanced Cancel/Reopen/New-Revision contract; automatic **physical** stock borrowing. **All Phase 2 — not designed here.** **(F1-7N-FA-3A.0 clarification:** these are the PHYSICAL/execution concerns — reservation, movement, ownership transfer. The **analysis-only planning surplus netting** for the *recommendation* is Phase-1 AUTHORIZED and commits nothing physical. **CORRECTED (F1-7N-FA-3B-R0; §44):** the LIVE monthly post-coverage residual / Net Order Need is owned by `KMTPP.projectTimePhasedSupply`; §41/`KMFSR` is a pure surplus-reallocation library that adjusts factory coverage feeding KMTPP once integrated (FA-3B1/B3) — it is not itself the live monthly Net-Order-Need producer.)

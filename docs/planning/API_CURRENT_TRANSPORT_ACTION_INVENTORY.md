...
**WSR-1 reconciliation (2026-08-04) of the five missing Weekly handlers** (see `WEEKLY_SHIPPING_PLAN_HANDLER_RECONCILIATION.md`): `git log -S` proves **none were ever committed** — they are router+adapter stubs (added 2026-07-28, `b7a216d`) with contract comments; no UI caller. Refined status: `getWeeklyPlanRateCandidates` — engine `shippingRoughRateCandidates_` (`17:629`) SOURCE-PROVEN_CURRENT but orphaned (handler NOT_IMPLEMENTED) → CONTRACT_PENDING_IMPLEMENTATION; `updateShippingPlanRationale` / `selectShippingPlanCarrier` — NOT_IMPLEMENTED, contract canonical (`WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1A), data model provisioned → CONTRACT_PENDING_IMPLEMENTATION; `combineShippingPlans` / `uncombineShippingPlans` — **SPEC-SUPERSEDED** by the derived-`MULTI` model (B-2, §3.1A/§4.3) → DEAD_OR_LEGACY_CANDIDATE / REQUIRES_USER_DECISION. First Weekly read+status slice is **NOT blocked** by any of the five.

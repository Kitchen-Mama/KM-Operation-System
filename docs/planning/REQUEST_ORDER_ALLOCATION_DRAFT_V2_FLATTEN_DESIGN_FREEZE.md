...
## 42. R6E1A — Cumulative release manifest correction + complete Shipping Plan payload fingerprint (2026-08-22)

Focused release-blocker correction on top of local `7521050`. `origin/main` had advanced to `7521050` via the USER's manual push of the R6E1 hand-off (reflog: `update by push`) — I never pushed; the precondition's "origin/main remains 6e0e9b2" was simply stale. This round adds one local commit on top; still NOT pushed. Bundle UNCHANGED. No production mutation / migration / Submit / Send / Inventory AI Plan; no flag changes.

### 42.1 (A) Cumulative frontend release manifest (blocker fix)
R6E1 unified only the files it changed in commit 7521050 and left **`request-order.js` on the stale `r6c-navlifecycle-20260822` token** — but `request-order.js` was materially changed at R6E (Site Confirm), so its R6E fix would never cache-bust on deploy. Audited the CUMULATIVE undeployed frontend set since the last verified live R6C1 release via `git diff --name-only d712989..HEAD`:
`assets/js/core/namespace.js`, `assets/js/api/operation-system-db-api.js`, `assets/js/api/km-api-foundation.js`, `assets/js/pages/inventory-replenishment.js`, `assets/js/pages/request-order.js`, `assets/js/app.js`, `index.html`.
Unified authority: `KM.RELEASE = 'r6e1a-shipping-release-20260822'` (namespace.js). Every one of the 6 cumulative changed JS assets now carries `?v=r6e1a-shipping-release-20260822` in index.html; NONE remains on r6c / r6d1 / r6e1. Load order unchanged (namespace → DB/API foundation → app/pages). `__roDebug().release`, `__kmLifecycleDebug().release`, and the new `__kmCapabilities().release` all read `window.KM.RELEASE`, so they agree with the loaded token. `lifecycle.js` / `home.js` are UNCHANGED since R6C1 → legitimately keep their r6c token; Home/Logo + nav-lifecycle fixes remain functional (R6C suite green).

### 42.2 (B) Cumulative backend sync manifest
`git diff --name-only f066924..HEAD` (permanent `.gs`, excluding tests/docs) yields exactly: `00_config.gs`, `01_router.gs`, `03_master_data_handlers.gs`, `11_shipping_plan_handlers.gs`, `TEMP_migrate_request_order_draft_v2.gs`. No other permanent `.gs` changed between `f066924` and this commit. `TEMP_` remains paste-run diagnostic/migration tooling (never permanent runtime; USER-run from the Run menu). Bundle `6c47f4358e…` unchanged (52 modules) — no bundled `core/supply-*.js` touched.

### 42.3 (C) Complete canonical payload fingerprint (replaces the narrow signature)
The R6E1 signature (`[country|ship_from|destination|method|marketplace|sku|requested_qty]`) could not detect a changed carrier / last-mile / customs / cost / note / snapshot. Replaced with `shippingPlanCanonicalFingerprint_` (version `spfp-1`): a canonical-JSON structure over EVERY persisted user/business field — header (company [server-resolved first], country, marketplace, ship_from, source_warehouse_id, ship_from_type, destination, destination_warehouse_id, destination_type, shipping_method, last_mile_delivery, customs_type, carrier_id, carrier_unit_rate, carrier_rate_type, import_duty_treatment, estimated_freight_cost, estimated_duty, estimated_customs_fee, estimated_total_cost, currency, plan_name, note, source) and each line (sku, site_sku, marketplace, requested_qty, approved_qty, plan_carton_qty, units_per_carton, carton_cbm, cbm, gross_weight, net_weight, all snapshot_* fields, source_page, source_reason, inventory_snapshot_date, note) — EXCLUDING server identity/audit fields (shipping_plan_id, shipping_plan_line_id, created_at/updated_at, created_by/updated_by, status/plan_version/batch/submission timestamps). Rules honored: company resolved before fingerprinting; null/blank canonicalized to `null`; numbers coerced so `400 === "400"`; objects already serialized to canonical JSON strings (shippingPlanSnapshotValue_); lines sorted by identity + headers sorted by route before hashing; canonical JSON (fixed key order) not delimiter concat; FNV-1a hash; version-tagged. The writer computes the fingerprint from the server-DERIVED would-be batch (the exact objects it writes) so a true retry hashes equal. Cross-day retries (plan_name/quote include `today`) intentionally CONFLICT — the fail-closed direction.

### 42.4 (D) Existing/partial-state classification
`shippingPlanClassifyBatch_` states for a provided key: no header → CREATE; two headers under the key sharing a route (corrupt double-write) → DUPLICATE_CONFLICT; header(s) with zero lines while the payload has lines → COMMITTED_UNVERIFIED; persisted lines schema-incomplete (pre-migration rows lacking the canonical marketplace/snapshot columns — the column-free "legacy/unknown fingerprint version" signal) → RECONCILIATION_REQUIRED (never a false REUSE); complete fingerprint equal → REUSED; complete fingerprint different → SUBMIT_EXECUTION_DUPLICATE_CONFLICT. It never reconstructs an incomplete payload and calls it equivalent.
...
## 44. R6A1 — Request Order Flat V2 Send path: legacy header authority removed; eligible-tier contract + downstream idempotency verified (2026-08-22)

Precondition satisfied: clean main containing R6F + R6E1A (HEAD = origin/main = 91a845a, 0/0). Three flags frozen (flat V2 true / site confirm false / inventory generation false). NO live DB change, NO live Send, NO Shipping Plan migration, NO Inventory AI Plan, legacy Draft tables untouched. Bundle UNCHANGED (fix is in non-bundled 15_ + frontend + TEMP; KMRDV2/KMRDV2P core untouched).

### 44.1 (A) Exact root cause + throw chain
`handleSendRequest` (request-order.js:3160) → first live write `DB.upsertRequestOrderAllocationDraft` (request-order.js:3311 canonical / :3322 manual) → router `upsertRequestOrderAllocationDraft` (01_:432) → **`handleUpsertRequestOrderAllocationDraft_` (15_:117) / `raUpsertDraftHeaderCore_` (15_:161)** → `procurementEnsureSheet_(ss,'request_order_allocation_drafts', REQUEST_ORDER_ALLOCATION_DRAFTS_HEADERS_)` (15_:125,163) → `prodRequireSheet_` (29_:53) → `KMSAFE.classifySchemaMismatch` → **`HEADER_MISSING`**. The passed authority was the **legacy 26-col** `REQUEST_ORDER_ALLOCATION_DRAFTS_HEADERS_` (15_:29) whose cols 7-8 are `category_snapshot`/`series_snapshot`; the live tab is the 53-col Flat V2 schema (`KMRDV2.V2_HEADERS`, hash 3b0e930b) which excludes those two → they are reported missing → `PRODUCTION_SAFETY:HEADER_MISSING [request_order_allocation_drafts]`. The header upsert had **NO** `requestOrderDraftV2FlatCutoverEnabled_()` branch (only the *submit* handler at 15_:285 did). It threw before Send ever reached 13_ `createRequestOrderDraft`. The failed attempt made **ZERO durable downstream writes** (throw is in the pre-write ensure). The live 53-col table is correct; the authority selection was wrong.

Exact legacy-only headers incorrectly selected (present in legacy authority, absent from V2): `category_snapshot`, `series_snapshot`.

### 44.2 (B) Flat V2 authority fix
Added `raDraftsHeadersAuthority_()` (15_): under `requestOrderDraftV2FlatCutoverEnabled_() && KMRDV2.V2_HEADERS`, returns the 53-col V2 authority; else the legacy constant. Both header-upsert ensures (15_:125, 15_:163) now call it — authority is selected **BEFORE** any `prodRequireSheet_` (mirrors the sibling loader 23_ `rprReadTable_`:58-62). The header upsert writes only by header NAME (`procurementAppendByHeader_`/`setCol`), so `category_snapshot`/`series_snapshot` simply drop against the V2 tab (never required, never written) and NO `request_order_allocation_draft_lines` row is touched by this step. flag=false → byte-identical legacy authority. A genuinely non-V2 live schema still fails closed (V2 columns missing). No silent fallback to the legacy engine while flag=true.

### 44.3 (C) Eligible-tier Send contract
The tier-eligibility contract `KMRDV2.explodeSendRequestLinesFromDto` (supply-planning-request-draft-v2.js:226; wrapper `KMRDV2P.buildSendRequestLines`) — previously with no runtime caller — is now the TESTED authority: per tier, skip `order_qty <= 0` and skip `status === 'cancelled'`; emit one line per eligible tier `{ sku, company, country, marketplace, request_bucket, request_month, requested_qty, units_per_carton, carton_qty, request_allocation_draft_id }` (never one header per tier — the existing Series grouping is preserved). Frontend `_roV2BuildSendLinesFromFlat_` (request-order.js:3794) delegates to it. NO_ELIGIBLE_SUBMITTED_DRAFTS is a clean zero-write result (request-order.js `!drafts.length` returns before any DB call — never HEADER_MISSING/generic). Zero-qty tiers → no line; draft/cancelled tiers → not sent; a fully-submitted draft sends all submitted positive tiers.

### 44.4 (D) Downstream mapping + lineage (already sound; verified)
`createRequestOrderDraft` (13_ `roCreateRequestOrderCore_`) maps the canonical flat draft → `request_orders` (+1) → `request_order_lines` (+N) → `request_order_line_sources` (+N), N = submitted tiers with order_qty>0. Each `request_order_line_sources` row carries `request_order_id`, `request_order_line_id`, `request_allocation_draft_id` (13_:67 additive FK), tier/month lineage (`tier_type`/`source_bucket`/`source_month`), SKU + scope. Send stops at Request Order — no PO (`handleCreatePurchaseOrderFromRequest_` is a separate action, never invoked by Send).

### 44.5 (E) Write-boundary (verified)
13_ validates all three downstream sheets (`procurementEnsureSheet_` at 13_:736/737/738) BEFORE the first append (line loop 13_:780+) → schema failure = zero writes; compensation `roDeleteRequestOrderById_` (13_:686) on a partial write error → `REQUEST_ORDER_WRITE_FAILED … compensated:true`. 15_ (B fix) now also fails closed correctly under cutover.

### 44.6 (F) Idempotency (verified) + Site Confirm scope
13_ `handleCreateRequestOrderDraft_` under LockService: execution key `roExecutionKey_` (ROEXEC-<sha256>) find-or-reuse — 0→CREATE, 1→REUSED (zero writes), >1→`REQUEST_ORDER_EXECUTION_DUPLICATE_CONFLICT`; concurrent identical → one Request Order. Site Confirm false (R6E) bypasses ONLY the Site Confirm gate (request-order.js:3169 wrapped in `if (_roSiteConfirmRequired())`); submitted-state, positive-line, canonical identity, schema, optimistic token, execution key, duplicate protection all stay mandatory.

### 44.7 (G) Structured Send error
`_roSendErrorMessage_` (request-order.js) — business message + technical code/affected table; never `[object Object]`; maps HEADER_MISSING/schema, DUPLICATE_CONFLICT, COMMITTED_UNVERIFIED (do-not-retry), token/version conflict; failed Send does NOT clear the working Draft; no false "DB Draft retained" claim; REUSED is shown as already-sent (existing success path).

### 44.8 (H/I) Diagnostic + validators + deployment
`TEMP_R6A1_DIAGNOSE_REQUEST_ORDER_SEND_PATH()` (read-only): target, three flags, 53-col schema+hash, selected authority (FLAT_V2), authority-before-guard, legacy-only headers, `_lines` dependency, downstream counts/hashes, eligible submitted-tier counts, execution-key collision, zero-write evidence for the observed failure, expected 1/N/N deltas, verdict (READY_FOR_CONTROLLED_REQUEST_SEND / NO_ELIGIBLE_SUBMITTED_DRAFTS / DOWNSTREAM_COLLISION / LEGACY_AUTHORITY_PRESENT / SCHEMA_MISMATCH / HALT). `TEMP_R6A1_VALIDATE_AFTER_REQUEST_SEND()` + `TEMP_R6A1_VALIDATE_REQUEST_SEND_REUSE()` (read-only): downstream lineage, zero duplicate execution-key groups, zero legacy Draft-Line dependency. **Deployment manifest:** frontend `index.html` + `request-order.js` (+ unified `r6a1-request-send-20260822` token; namespace.js KM.RELEASE); backend sync `15_request_allocation_handlers.gs` + `TEMP_migrate_request_order_draft_v2.gs`. No bundle rebuild. **HALT/deferred:** a full server-side rewire of the downstream line derivation to call `buildSendRequestLines` inside the .gs writer is NOT done (the existing 13_ downstream is idempotent + lineage-correct and the frontend already builds the equivalent eligible-tier lines; rewiring it server-side is unverifiable with no live Send). The controlled live Send + the two validators remain USER-owned.

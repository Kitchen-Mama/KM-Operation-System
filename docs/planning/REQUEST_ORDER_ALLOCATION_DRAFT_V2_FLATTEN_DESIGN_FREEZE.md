...
**20.3 Paste-ready `.gs` helpers** (`assets/specs/active/apps-script/TEMP_migrate_request_order_draft_v2.gs`; TEMP / not permanent runtime): `TEMP_migrateRequestOrderDraftV2_({execute})` — DEFAULT `execute:false` (dry-run, read-only, logs plan+report+samples); `execute:true` writes ONLY `request_order_allocation_drafts_v2` (53 headers + 26 rows) and fails closed if that tab already exists with data/other schema; it NEVER touches the legacy tabs, NEVER renames/deletes, NEVER flips the flag/deploys. `TEMP_validateRequestOrderDraftV2Staging_()` — read-only, calls `validateStaging`. Both delegate all semantics to KMRDV2/KMRDV2P.

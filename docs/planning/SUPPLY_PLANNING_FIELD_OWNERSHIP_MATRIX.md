...
| §41 factory surplus reallocation LIBRARY (§44) | `supply-planning-surplus-reallocation.js:KMFSR` — `projectSurplusReallocation` (standalone) + **`reallocatePreallocatedFactorySupply` (FA-3B1 adapter)** | ∅ | ∅ | FA-3B3 wires KMMSA/KMAR coverage → adapter → KMTPP | ∅ | `…-f1-7n-fa-3a` (270) + `…-preallocated-f1-7n-fa-3b1` (99) | IMPLEMENTED_NOT_PRODUCTION_CONNECTED | live initial factory allocation = **KMMSA/KMAR** (§44.1); adapter runs §41 over preallocated coverage with NO second §40 allocation (spy-proven); shared §41 core, no formula fork |

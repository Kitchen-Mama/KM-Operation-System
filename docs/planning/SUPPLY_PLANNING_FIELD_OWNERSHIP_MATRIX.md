...
| company-level pooling orchestration (PLANNING-ONLY, §41) | `supply-planning-surplus-reallocation.js:KMFSR.projectSurplusReallocation` (IMPLEMENTED FA-3A — PURE, NOT CONNECTED) | ∅ | ∅ | source-facts (netOrderNeed) | ∅ | `supply-planning-surplus-reallocation-f1-7n-fa-3a` | PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED | analysis-only iteration engine (Phase-1), 270 unit assertions; physical borrowing = Phase-2 → F1-6/FA-3A |
...
| **PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED** | company-pooling orchestration — Factory Surplus Reallocation (PLANNING-ONLY, §41 / §41.5A / §43 / F1-6, `KMFSR.projectSurplusReallocation`, FA-3A; 270 assertions; NOT wired to monthly/Gap/Weekly/Request-Order) |
...
| Ongoing-Order site allocation | `supply-planning-ongoing-order-projection.js:KMOOP.projectOngoingOrderSupply` — A1 = `request_order_line_sources.requested_qty` (immutable, per site); A2 = within-company monthly FC share (`KMPCX.forecastShareQty` basis, assembly; NOT weekly `demandWeight`, NOT KMDA `50_` warehouse ratios) | PURE_RUNTIME_IMPLEMENTED_NOT_CONNECTED (FA-3B0; 55 assertions) | single-company PO line; §43 FLOOR + residual; no cross-company denominator / equal-split / name heuristic; projects only `MAX(0, ordered − completed)`; completed/shipped excluded; not wired to Monthly/Gap/Weekly/Request-Order |

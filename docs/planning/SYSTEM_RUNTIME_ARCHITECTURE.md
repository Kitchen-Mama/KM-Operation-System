...
> - **Status:** Reviewed — B-1 / B-2 / B-3 RESOLVED (decision only; owners §8A.1 / `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md` §3.1 / §3.1B); **B-4 CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED**; **B-7 RESOLVED (2026-08-02, decision only — recommendation-cycle Composite Natural Key / Submit commitment boundary, §7A; Runtime not implemented)**; **B-5 RESOLVED (2026-08-03, decision only — `request_order_lines` / `request_order_line_sources` grain + quantity authority + Monthly SKU split + Recommendation→Request writer boundary; owner `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9; writer / persistence / Runtime NOT implemented)**; B-6 / B-8 UNRESOLVED. **Phase 2B Calculation Pure Runtime = FUNCTIONALLY COMPLETE / TEST VERIFIED (2026-08-03, Round 11A closure — see §7B); Production Integration (data acquisition / persistence / scheduler / LockService / Submit-guard / deployment) = PENDING; the whole Recommendation Runtime is NOT complete.** **Recommendation Persistence / Orchestration Contract = FROZEN (2026-08-03, Phase 2C Round 1A — decision only; public command + concurrency + Submit boundary owned here at §7C, full contract at `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Orch); implementation NOT STARTED.** **Persistence CORE = IMPLEMENTED / TEST VERIFIED (Round 1B, pure module, 96 tests). Production Persistence Adapter / Repository Contract = FROZEN (Round 1C); Repository SLICE 1 IMPLEMENTED / TEST VERIFIED (Round 1D, 74 tests, source mirror — NOT deployed). LockService + optimistic-concurrency write boundary IMPLEMENTED / TEST VERIFIED (Round 1E, `supply-planning-persistence-locking.js`, 96 tests — NOT deployed). Plan Builder + Persistence Plan Builder + Apps Script module bundle/port + locked orchestrator (with keyed-delta write) IMPLEMENTED / TEST VERIFIED (Round 1G, 50 / 44 / 38 / 37). **Locked-path enforcement + locked user-edit + terminal-guard unification IMPLEMENTED / TEST VERIFIED (Round 1H, `supply-planning-user-edit.js` 37 + route-inventory audit 32 + `25_recommendation_user_edit.gs` + `15_`/`16_` locked adapters): every production-reachable Recommendation-Draft mutation route is lock-enforced with a shared KMPR terminal-status vocabulary; recommended_qty snapshot + user decisions preserved; no unlocked bypass (route inventory: 0 FORBIDDEN_PUBLIC_WRITE); full-table writer not reachable from a locked route; shipping remains deployment-gated (lock+terminal enforced, token/KMUE unification pending). Source mirror, NOT deployed; source-facts reader PENDING.** Scheduler / Trigger / Request writer / Weekly-Plan / Submit NOT IMPLEMENTED.**
...
> - **Last Reviewed:** 2026-08-03 (Phase 2C Round 1E — Recommendation Persistence **LockService + optimistic-concurrency write boundary** IMPLEMENTED / TEST VERIFIED (pure orchestrator `supply-planning-persistence-locking.js`, 96 fake-lock assertions; `applyPersistencePlanWithLock` source mirror; reload-under-lock + token revalidation + zero-write conflict/blocked/terminal guards + release-in-finally); NOT deployed. Prior: Round 1D Repository SLICE 1 IMPLEMENTED / TEST VERIFIED (74); Round 1C adapter/§7C repo model B + script-lock reality + optimistic token + additive schema; Round 1B persistence core (96); Round 1A §7C public-command + concurrency + Submit; B-5 Decision Round; Round 11A Calc closure; §7B).
> - **Depends On:** DB Map, System Flow, Calculation Rules (formulas), domain specs.
> - **Blocked By:** Batch B — **B-4 Qualified Incoming Runtime prerequisites** — canonical Shipment quantity reader, destination identity read, normalized supply candidates, authority/admission classification, deduplication, ten-gate Runtime and Line Runtime integration; the B-4 **contract itself is resolved** (see `SUPPLY_CHAIN_SYSTEM_FLOW.md` §11 B-4). **B-1 / B-2 / B-3 RESOLVED (decision only); B-4 CONTRACT RESOLVED — RUNTIME NOT IMPLEMENTED; B-7 RESOLVED (decision only — recommendation-cycle Composite Natural Key / Submit commitment boundary, §7A); **B-5 RESOLVED (2026-08-03, decision only — `request_order_lines` / `request_order_line_sources` grain + quantity authority + Monthly SKU split + Recommendation→Request writer boundary; owner `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9; writer / persistence / Runtime NOT implemented)**; B-6 / B-8 UNRESOLVED.**
...
- **A recommendation scheduler entry point MUST:** accept no required arguments; enforce source-data readiness; be idempotent by recommendation cycle (see Duplicate-run protection below; the persisted cycle-key mechanism is **B-7 RESOLVED — a Composite Natural Key, decision only, Runtime not implemented**); reuse the existing Active Draft on Retry/Resume; never overwrite user quantity; never mutate a Submitted business record; never report success after a partial failure.
...
**Duplicate-run protection (idempotent):** there must be **one active recommendation batch per recommendation cycle + Scope** (a cycle = one weekly run for Shipping, one monthly run for Order); repeated/retried execution for the same cycle must be **idempotent** (no duplicate Draft headers/lines, no reset of user-edited fields); a failed partial run **must not report success**. The **persisted cycle / unique-key mechanism** is **RESOLVED — B-7 (2026-08-02, decision only)** (`SUPPLY_CHAIN_SYSTEM_FLOW.md` §11).

**Recommendation-cycle identity (CANONICAL — B-7, decision only; Runtime NOT implemented):**
- **Identity = a Composite Natural Key** (Option B). **No `recommendation_cycle_id` and no schema change now**; a future surrogate `recommendation_cycle_id` is permitted for **DB hardening only** and never replaces the natural-key business identity. **No DB unique index is added here** — uniqueness is Runtime-enforced (deterministic lookup + project LockService + fail-closed).
- **One Active Draft only** per `recommendation_type + planning_cycle + business scope`:
  - **Weekly** = `shipping_allocation_drafts` + `planning_cycle` (ISO `YYYY-Www`, Asia/Taipei) + company + country + marketplace + source_page + draft_version (destination is line-level).
  - **Monthly** = `request_order_allocation_drafts` + `planning_cycle` (`YYYY-MM`, Asia/Taipei) + company + country + marketplace + draft_purpose + draft_version (Monthly `sku` split-grain **deferred to B-5**).
- **Scheduler / Retry / Resume** MUST reuse the existing Active Draft, MUST NOT create a duplicate active Draft, and MUST NOT overwrite user-edited `planned_qty` / `order_qty`.
- **Manual Recommend / Regenerate** is an explicit rebuild with the latest data: the latest recommendation becomes the current Active Draft, `recommended_qty` is recalculated, `draft_version` increments; **if manual quantities exist, user confirmation is required before regenerating planning quantities**.

> **CANONICAL BUSINESS RULE (permanent, B-7):** **Submit is the business commitment boundary. A Recommendation Engine (manual or automatic) shall never mutate a Submitted Business Record.** After Submit, any further modification must follow **Reject / Cancel / Reopen / New Revision** (release mechanics = **B-8**), never Recommendation. External OMS-created Inbound / Outbound records are never Recommendation authority and remain outside Recommendation until explicitly linked/adopted into KM Canonical Shipment per the **B-4** authority contract. Recommendation is an operational assistant only; business execution authority begins only after Submit.

**No DB column, key, or index is decided or added here; composite-lookup / LockService / scheduler / writer / Regenerate / Submit-guard Runtime is NOT implemented.**
...
| Recommendation Draft persistence writer | **PENDING** (schema + body-driven handler source exist — §7A table — but no recommendation calc engine / no automatic write path). Its **target grain + input/output/idempotency identity are now B-5-RESOLVED** (`REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.9): line key `(request_order_id, company, sku, request_bucket)`, one line → 1..N demand-provenance sources, `approved_qty` single authority, three-level idempotency identity — **the writer itself remains unimplemented**. |
| Recommendation→Request writer (Send Request → `request_orders` / `_lines` / `_line_sources`) | **PENDING** (contract identity frozen by B-5 §3.9 — aggregate site-level draft lines to `company+sku+request_bucket` lines, demote site dims to 1..N sources; lock/transaction realization + Request→PO atomicity = **B-6**; source cancel/release = **B-8**) |
...
## 7C. Recommendation Persistence / Orchestration — Public Command & Concurrency Boundary (FROZEN — Decision Only, 2026-08-03)

> **Status: CONTRACT FROZEN — NOT IMPLEMENTED.** Full contract owner = `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Orch (Phase 2C Round 1A). This section is the permanent home for the **public command boundary, concurrency boundary, Submit immutability, and scheduler/manual parity**. No scheduler / writer / LockService / persistence is implemented (§7B Production-Integration table stays PENDING). Does not decide B-6/B-8; does not reopen B-7.

**Public commands (side-effecting; smallest coherent set):** `runWeeklyShippingRecommendation()` / `runMonthlyOrderRecommendation()` (no-arg scheduled adapters, §I) → both funnel through the **one shared core** `generateRecommendationDraft(cmd)` (so a scheduled run and a manual Recommend/Regenerate produce the **identical** result — **scheduler/manual parity**), plus `submitRecommendationDraft(cmd)` (the commitment boundary). Downstream handoffs are owned elsewhere: Monthly `Send Request` = the B-5 §3.9 writer; Weekly `Create/Update Weekly Plan` = `WEEKLY_SHIPPING_PLAN_MAPPING_SPEC.md`. `recommendation_type ∈ { WEEKLY_SHIPPING, MONTHLY_ORDER }` (Emergency §36.4 = a MONTHLY_ORDER via `generation_type`, not a new type).

**Concurrency boundary:** acquire a project `LockService` lock on the Active-lookup key (`recommendation_type + planning_cycle + business scope`) **before** the resolve→validate→upsert→totals→mark-complete critical section; calculation (pure §39/§40) may run outside the lock, but a **second Active-Draft validation under the lock is mandatory before any write**; **no write on lock failure** (abort safely — another run holds it). At most **one Active Draft** per key (Active = `{draft, site_confirmed}`); a duplicate-active state **blocks the run** (no silent pick, no latest-wins, no auto-repair). Google Sheets provides **no ACID transaction** — the write order is a **logical transaction** (run-metadata → header → lines → reconcile → lineage → totals → mark-complete → release) with **no empty-success** before all rows are consistent; a partial write is resumable by reusing the same `calculation_run_id`.

**Submit immutability (permanent, B-7):** a Recommendation Engine (manual or automatic) shall **never mutate a Submitted Business Record**; after Submit, change follows Reject / Cancel / Reopen / New Revision (mechanics = **B-8**). `draft_version` increments **only** on a successful Manual Regenerate (never on scheduler retry/refresh); user-edited `planned_qty`/`order_qty` are **never** overwritten by a scheduled refresh; `recommended_qty` is immutable within a `draft_version`. **The pure calculation lane stays side-effect-free; only the orchestration is side-effecting.**

**Core state machine vs production repository (Round 1B/1C).** The Round 1B pure state machine (`assets/js/core/supply-planning-persistence.js`, IMPLEMENTED / 96 tests) is deterministic and side-effect-free; the **production repository adapter** (Round 1C, FROZEN — Decision Only, `RECOMMENDATION_RUNTIME_IMPLEMENTATION_SPEC.md` §Persist-Adapter; **NOT IMPLEMENTED**) maps it to Sheets via model **B**: load a snapshot → run the pure core → derive a deterministic **Persistence Plan (diff)** → apply it under the lock (repo = `loadActiveDraftContext` / `loadDraftSnapshot` / `loadIncompleteRun` reads + one `applyPersistencePlan` write). **LockService reality:** Apps Script offers only Script/Document/User locks — **no native per-key lock**; the pattern is `getScriptLock()` + `tryLock(30000)` + `releaseLock()` in `finally` (matching `05_/22_*.gs`), with the **logical Active-Draft key preserved in a revalidation/conflict check, not in the lock primitive**. **Optimistic concurrency token** = `{ draft_version, userEditFingerprint }` (draft_version alone cannot detect a line-level user edit); captured before the lock, re-checked under it; on mismatch a refresh retries and a regenerate re-confirms — **never overwrite on mismatch**. Additive schema required (not added): `user_edited`/`user_edited_by` on both line tables + a `recommendation_calculation_runs` journal table (stage markers prove which stage completed → resumable). **Round 1D/1E status:** the repository readers + plan validation/application (Round 1D) and the LockService + optimistic-concurrency orchestrator (Round 1E — `executeLockedPersistence`: acquire → reload-under-lock → revalidate token → apply → release-in-finally, with zero-write conflict/blocked/terminal guards) are **IMPLEMENTED / TEST VERIFIED** as pure modules (74 + 96 assertions) and mirrored in `23_*.gs` (`applyPersistencePlanWithLock`), **NOT deployed**; Scheduler / Trigger / writer / Submit remain PENDING (§7B Production-Integration table unchanged).

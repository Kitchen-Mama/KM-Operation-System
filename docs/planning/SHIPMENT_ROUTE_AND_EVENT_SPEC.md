...
- **2026-08-01 (Round 4D-C — cross-reference only; version retained):** Delivered ≠ Received and the shipment-ETA authority feed the external-origin admission model — carrier / OMS / WMS ETA and `delivered` events are **execution evidence** that update a **linked** KM operation only through validated handlers; they **never** auto-admit an **unlinked** external record into planning (owner `SUPPLY_CHAIN_SYSTEM_FLOW.md` §12 / `SUPPLY_PLANNING_CALCULATION_RULES.md` §38). No content owned here changed.

...
**44.9 Status + slices.** KMFSR / KMOOP status = **IMPLEMENTED_NOT_PRODUCTION_CONNECTED** until corrected FA-3B lands. Slices: **FA-3B1** = KMFSR preallocated-input §41 adapter — **IMPLEMENTED (`reallocatePreallocatedFactorySupply`, pure, not production-connected)**; **FA-3B2** = KMOOP→KMTPP incoming adapter — **BLOCKED** (FA-3B2-PRE audit proved no owner yields an ongoing-PO arrival ETA without a semantic choice / invented transit; awaiting USER decision `D-ONGOING-TIMING`, see §44.3); **FA-3B3** = Architecture-A wiring KMMSA/KMAR→KMFSR adapter + KMOOP coverage→KMTPP (NO until B1+B2); **FA-3B4** = persistence/diagnostics validation. No second engine; KMMSA/KMAR/KMTPP never replaced.

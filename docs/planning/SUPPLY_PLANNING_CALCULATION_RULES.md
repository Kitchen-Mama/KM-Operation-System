...
**44.3 KMOOP repositioned = pure §42 ongoing-order site-projection LIBRARY.** KMOOP produces integer site-attributed `notYetReceivedCommittedQty` (A1 request-source share; A2 same-company monthly FC share; §43 FLOOR + residual). It does **NOT** own monthly timing. Live integration (FA-3B2) maps KMOOP site allocations into KMTPP incoming facts `{incomingId, eta, qty, sourceType:'ONGOING_ORDER', tier}`, where `tier` derives from an ETA-month via the KMTPP timing owner. **Ongoing timing authority (OPEN):** the availableDate/ETA for not-yet-received PO supply is not yet resolved from an existing canonical fact. Candidates: `expected_ship_date` / `expected_completion_date` (purchase_order_lines) combined with the existing canonical lead-time owner (`55_api_v1_lead_time_owner.gs`) to derive a destination-arrival ETA. Until an FA-3B2 audit proves this maps to KMTPP WITHOUT inventing lead time, carry `ONGOING_ORDER_MONTHLY_TIMING_AUTHORITY_MISSING` as a blocker; do NOT invent lead-time math.
...
**44.9 Status + slices.** KMFSR / KMOOP status = **IMPLEMENTED_NOT_PRODUCTION_CONNECTED** until corrected FA-3B lands. Slices: **FA-3B1** = KMFSR preallocated-input §41 adapter — **IMPLEMENTED (`reallocatePreallocatedFactorySupply`, pure, not production-connected)**; **FA-3B2** = KMOOP→KMTPP incoming adapter (READY only if an existing PO timing fact maps to KMTPP without invention — currently blocked by `ONGOING_ORDER_MONTHLY_TIMING_AUTHORITY_MISSING`); **FA-3B3** = Architecture-A wiring KMMSA/KMAR→KMFSR adapter + KMOOP coverage→KMTPP (NO until B1+B2); **FA-3B4** = persistence/diagnostics validation. No second engine; KMMSA/KMAR/KMTPP never replaced.

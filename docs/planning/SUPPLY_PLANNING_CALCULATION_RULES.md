...
**41.7 Net Order Need authority.** The existing `KMCALC.sumRemainingShortages` / `KMSF.resolveMonthlyRecommendationFacts` remain the **sole** Net Order Need owner (§12/§14/§32). §41 does **not** create a second formula — FA-3 connects the missing producer stage that feeds a genuinely post-reallocation `remainingShortages` array into it, making `net_order_need_snapshot` post-reallocation.

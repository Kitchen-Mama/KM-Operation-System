...
**44.9 Status + slices.** KMFSR / KMOOP status = **IMPLEMENTED_NOT_PRODUCTION_CONNECTED** until corrected FA-3B lands. Slices: **FA-3B1** = KMFSR preallocated-input §41 adapter — **IMPLEMENTED (`reallocatePreallocatedFactorySupply`, pure, not production-connected)**; **FA-3B2** = KMOOP→KMTPP incoming adapter (KMOTA) — **IMPLEMENTED (pure, not production-connected)**: `D-ONGOING-TIMING` resolved (Option A, PHASE-1 anchor `expected_completion_date`, see §44.3); no arrival-ETA / transit / shipment dependency; count-once and receiver lineage preserved; timing isolated behind `resolveOngoingAvailableDate_` for a Phase-2 swap; **FA-3B3** = Architecture-A live wiring — **BLOCKED (F1-7N-FA-3B3 AUDIT, 2026-08-20; two USER decisions required, see §44.11)**. The read-only trace proved the live path does not read PO data at all and already owns a *different, designed-but-unwired* Ongoing-PO seam (`KMSF committedProduction` → `COMMITTED_PRODUCTION`), and that §41 wiring would require reworking the frozen R2G-B contended-factory conservation core; **FA-3B4** = persistence/diagnostics validation. No second engine; KMMSA/KMAR/KMTPP never replaced.
...
**FA-3B3 status:** `FA_3B3_IMPLEMENTED = NO`, `ARCHITECTURE_A_LIVE_INTEGRATION_READY = NO`. No runtime code, no Apps Script, no schema changed in this audit. Prerequisite libraries KMFSR/KMOOP/KMOTA remain IMPLEMENTED_NOT_PRODUCTION_CONNECTED and are unaffected.

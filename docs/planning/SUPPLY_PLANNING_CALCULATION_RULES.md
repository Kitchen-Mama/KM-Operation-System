...
- Ongoing (unshipped, still-committed) qty = `MAX(0, ordered_qty − shipped_qty)` — **NOT** `remaining_qty` (= `completed_qty − shipped_qty`, the available-to-ship subset). Shipped qty exits to `SHIPPED_IN_TRANSIT` (not auto-`factory_stock`).

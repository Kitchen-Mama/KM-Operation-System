...
**44.2 KMFSR repositioned = pure §41 surplus-reallocation LIBRARY.** KMFSR is NOT a replacement monthly factory allocator. Live integration invokes §41 surplus reallocation AFTER KMMSA/KMAR have produced the initial source-attributed factory coverage. `KMFSR.projectSurplusReallocation` runs its OWN §40 initial allocation (its Phase 3) and is therefore the **standalone-model** entry point only. **Required adapter (FA-3B1, not built this round):** a new entry point (conceptually `KMFSR.reallocatePreallocatedFactorySupply(input)`) that accepts PREALLOCATED per-receiver source-attributed factory coverage (`{demandKey, receiverKey, requiredByDate, projectedRequirementQty, eligibleFactoryWarehouseIds, initialAllocationBySource:{warehouseId→qty}}` + `calculationDate`) and runs ONLY the frozen §41 protect→releasable-surplus→eligibility→transfer core (KMFSR Phases 4–10) — **no second §40 allocation, no §41 math change**. Output: adjusted per-receiver factory coverage + `transferLedger`. KMTPP then consumes the adjusted `factoryCoveredQty`. Invariant: reallocation never increases total factory coverage beyond the physical pool (§43).
...
**44.9 Status + slices.** KMFSR / KMOOP status = **IMPLEMENTED_NOT_PRODUCTION_CONNECTED** until corrected FA-3B lands. Slices: **FA-3B1** = KMFSR preallocated-input §41 adapter (no timing dep → READY); **FA-3B2** = KMOOP→KMTPP incoming adapter (READY only if an existing PO timing fact maps to KMTPP without invention — currently blocked by `ONGOING_ORDER_MONTHLY_TIMING_AUTHORITY_MISSING`); **FA-3B3** = Architecture-A wiring KMMSA/KMAR→KMFSR adapter + KMOOP coverage→KMTPP (NO until B1+B2); **FA-3B4** = persistence/diagnostics validation. No second engine; KMMSA/KMAR/KMTPP never replaced.

...
```
Cadence is **monthly** (one recommendation per planning cycle, cycle key `YYYY-MM`); each new month creates a **new** cycle recommendation and **never overwrites** a prior month's Draft/Line. A single monthly run may produce **many** scope/SKU parent records (one per eligible scope), not one global parent. *(The exact scheduled day/time is Runtime scheduling config, `SYSTEM_RUNTIME_ARCHITECTURE.md` — not defined here.)* Uses existing fields only — **no new DB column**.

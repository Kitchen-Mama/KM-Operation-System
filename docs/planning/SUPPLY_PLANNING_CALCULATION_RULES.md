...
### 35A.7 Weekly demand-grain, survival, weight & factory-identity authority (CANONICAL — F1-7N-D0-A, 2026-08-19; USER-frozen; documentation only, NO runtime)

This closes the D0/D0-A authority gaps the assembler HALTed on. It is a **demand-adapter + identity-configuration** freeze only: it changes **no** Horizon/Gap formula, **no** §39/§40 contract, **no** `allocateOverseasSharedPool`/`allocateFactoryDeterministic`, **no** F1-7N-B/C0/C1, and **no** monthly logic. Executable guards: `assets/tests/weekly-ai-plan-demand-authority-f1-7n-d0-a-r1.test.js`.

**Count-once global invariant.** One physical unit of demand or supply is counted **at most once** across D18/D30/D45/D90, receivers, demands, Overseas/CN/TW pools, survival, `demandWeight`, source allocation, and persisted lines. Formally: `SUM(incremental window demand)` represents the cumulative demand curve exactly once; `SUM(window demandWeight for one lane) == canonical lane demandWeight`; survival is applied once per lane; a supply unit allocated to one `demandKey` is never re-allocated; Overseas consumed before Factory is not re-counted as CN/TW availability; CN consumed before TW is not left in TW residual; `SUM(per-source planned_qty) ≤ aggregate recommended_qty`; no pool > 100%.

- **Layered model (never conflated):** Horizon Engine → *cumulative* shortage `horizons[].gapQty` (UNCHANGED) · Weekly Demand Adapter → *incremental* need · Protection Adapter → survival-once + conserved `demandWeight` · Demand Ordering → Required-By → `allocation_priority` → stable keys · Source Allocation → Overseas → CN_YOUXIN → TW_SHENGYI → unresolved (§35A.2) · Cartonization → FLOOR (§35A.3) · Persistence → F1-7N-C1 K3 Weekly Draft.
- **Cumulative → incremental demand projection.** `incrementalNeed(n) = max(0, cumulativeGap(n) − runningMax(cumulativeGap(1..n−1)))`. Examples (MUST hold): `10/20/30/40 → 10/10/10/10`; `20/20/20/20 → 20/0/0/0`; `0/10/10/25 → 0/10/0/15`; decreasing anomaly `20/15/30/25 → 20/0/10/0`. Only `incrementalNeed > 0` becomes a weekly demand. This projection lives **only** in the Weekly demand adapter; the cumulative Horizon fact is unchanged.
- **18-day survival — count once.** `survivalNeedQty = ceil(18 × canonicalDailyDemand)` (existing KMAF/KMSF/KMCALC authority — no second formula). Grain = `sku + destinationWarehouseId` lane. Attach the **full** canonical `survivalNeedQty` to the receiver of the **earliest window with `incrementalNeed > 0`**; every later window gets `survivalNeedQty = 0`. All windows zero → no survival receiver. Never protect 18 days ×N windows.
- **`demandWeight` — count once (fixes the multi-window double-count).** Reuse the canonical §7 lane/site `demandWeight`. Split across active windows so the lane conserves it: `windowDemandWeight = canonicalLaneDemandWeight × incrementalNeed(window) ÷ Σ(active incrementalNeed)`; zero-need window → 0; total-need 0 → emit no active receiver. **Invariant: `Σ(window demandWeight) == canonical lane demandWeight`** — a site does not gain cross-site allocation share merely by having more active windows (`allocateOverseasSharedPool` sums receiver weights with no per-site grouping; `supply-planning-allocations.js:99-101`). Weight is for fair allocation only; **time priority stays on Required-By / `allocation_priority`**, never on weight.
- **Canonical `demandKey` = `{sku}|{destinationWarehouseId}|{windowCode}`** (normalized ids; no UUID/timestamp/row-index/label). Used identically by `weeklyPlanningFacts`, `overseasInput.receivers`, `factory.demands`, the source-allocation result, and the resolver joins.
- **Gap semantics.** `cumulativeGapQty = horizons[].gapQty` (unchanged) and `incrementalNeedQty` (adapter) are distinct fields. The monthly Order-Planning `calculatedGap` is **never** weekly demand. At the F1-7N-B boundary **only**, `calculatedGap := incrementalNeedQty` is a DTO-compatibility alias — it does not redefine the cumulative fact.
- **Factory identity configuration (USER-confirmed, exact `warehouse_id` — Phase 1, NO schema change).** Identity is derived **only** from the exact full `warehouse_id`, never from country / company / `warehouse_name` / `warehouse_code` token / id-token parsing:
  - `CN_YOUXIN → WH-TW-CN-FACTORY-YOUXIN` (`warehouse_code` `CN_YOUXIN`, `warehouse_type` `FACTORY`, `country` `CN`, `is_factory_warehouse=TRUE`, `is_active=TRUE`)
  - `TW_SHENGYI → WH-TW-TW-FACTORY-RES` (`warehouse_code` `TW_RES`, `warehouse_type` `FACTORY`, `country` `TW`, `is_factory_warehouse=TRUE`, `is_active=TRUE`)
  - `resolveWeeklyFactoryIdentity(warehouse_id) → CN_YOUXIN | TW_SHENGYI | UNKNOWN`. Each configured row must exist, be `FACTORY` + `is_factory_warehouse=TRUE` + `is_active=TRUE`, and never overlap. Unknown / overlapping / missing / non-factory / inactive → **FAIL CLOSED** (never classified, never country-inferred). (Phase 2 may promote this into a first-class `warehouses.factory_identity` column.)
- **`eligiblePoolTypes` unchanged** — reuse the existing §24 / KMAF FBA/THREE_PL eligibility; no invented cross-type fallback.
- **`formula_version = WEEKLY_AI_PLAN_V1`** (never `ORDER_PLANNING_GAP`) — identifies the full weekly contract: cumulative→incremental projection · survival-once · multi-window weight conservation · deterministic `demandKey` · Overseas > CN_YOUXIN > TW_SHENGYI · no double-count · ≤100% conservation · carton FLOOR · unresolved residual preserved. Any later semantic change requires a new version.
- **`source_data_as_of`** = the existing canonical facts' `maxAsOf(...)` owner — never `Date.now()` / execution / scheduler-run time (runtime execution time is separate).
- **Weekly ≠ Procurement.** The Weekly AI Plan writes **only** `shipping_allocation_drafts` / `shipping_allocation_draft_lines`. It creates **no** `request_orders` / `request_order_allocation_drafts` / `purchase_orders`; the unresolved factory shortage is an **informational** production/procurement signal, not an automatic commitment. Monthly procurement stays separate.
...
## 41. Factory Surplus Reallocation Orchestration Contract (F1-7N-FA-3A.0 freeze — owner-of-record)

**Status:** CONTRACT FROZEN (documentation) — **RUNTIME NOT IMPLEMENTED** (lands in FA-3A). This section is the owner-of-record for the *assembly* of the already-frozen primitives into the post-reallocation Net Order Need. It **authors no new formula** — it cross-references §12 / §14 / §32 / §32A / §35 / §40 and freezes only the orchestration order, the intermediate "releasable surplus" fact, and the planning-vs-physical boundary.

**41.1 Planning-only semantics (AUTHORIZED, Phase 1).** This orchestration is **ANALYSIS-ONLY / PLANNING-ONLY**. It computes recommendation math (`remainingShortageQty`, `reallocation_in/out_qty_snapshot`, `net_order_need_snapshot`, Suggested Qty, Recommendation-Summary diagnostics) over DERIVED planning-availability facts. It **MUST NOT** mutate, reserve, deduct, move, or reassign any physical factory pool, and grants no ownership transfer. Physical cross-company borrowing/reservation is **Phase 2 / [[D-4]]** (`PHYSICAL_CROSS_COMPANY_RESERVATION_DEFERRED`). Cross-company netting here is authorized **only** as analysis (see DECISION_REGISTER PART B, resolved 2026-08-20). PHYSICAL CONSERVATION (per-company, FM5-R2A) ≠ PLANNING NETTING — both hold simultaneously.

**41.2 Orchestrator responsibility (F1-6, planning-only).** Reserved pure module `assets/js/core/supply-planning-surplus-reallocation.js` (namespace **KMFSR**; primary `KMFSR.projectSurplusReallocation(input)`). It WRAPS the frozen allocators (`allocateFactoryDeterministic` §35/§40, `allocateFactoryCrossCompany`) and the §32A primitives (`evaluateReallocationEligibility`, `feasibleReallocationQty`, `applyFeasibleReallocation`) and terminates in §12 `sumRemainingShortages` → `netOrderNeed`. It does **not** re-derive §40 and does **not** re-order the frozen allocators. (Distinct from the existing composition-glue `supply-planning-recommendation-orchestrator.js` — different responsibility, no name reuse.) **Production role (CORRECTED — §44):** in the LIVE Architecture-A monthly engine the canonical initial factory allocation is **KMMSA/KMAR** (not §40 `allocateFactoryDeterministic`); therefore `KMFSR.projectSurplusReallocation` — which runs its OWN §40 initial allocation at its Phase 3 — is the **standalone-model** entry point, NOT the live monthly path. Live integration (FA-3B3) uses KMFSR as a pure **§41 surplus-reallocation library** over the ALREADY-allocated KMMSA/KMAR coverage via a preallocated-input adapter (FA-3B1); it MUST NOT run a second factory allocation on that path.

**41.3 Frozen ordered chain.** `raw factory supply → canonical initial allocation (§35/§40, pool conserved) → receiver initial factory attribution → protect the receiver's own actionable T1–T3 requirement → releasableSurplusQty → legal cross-site/analysis-netting reallocation (§32A) → remainingShortageQty → Net Order Need (§12) → carton rounding / Suggested Qty (§14/§31)`. **Initial allocation precedes reallocation.** A site's initially-attributed factory stock is NOT automatically an order reduction for that site — its own chronological requirement is protected first.

**41.4 Definitions.**
- `protectedFactoryQty` = the portion of a receiver's initial factory attribution required to keep its actionable T1–T3 requirements non-negative (chronological).
- `releasableSurplusQty` = `MAX(0, initialFactoryAllocationQty − protectedFactoryQty)`. A receiver may donate ONLY this.
- `reallocatedOutQty` (donor) / `reallocatedInQty` (receiver): the analysis quantity legally transferred, capped by §32A.
- `remainingShortageQty` = receiver shortage after all legal reallocation.

**41.5 Tier rule (preserves §32A / FA-2 freeze).** Actionable tiers = **T1, T2, T3 only**; **T4 is visibility-only and MUST NOT participate** in allocation or reallocation (`_reallocationTierRank` → 0 for T4). Donor protection: `donorRank ≤ receiverRank` (both > 0) — earlier/same-tier surplus may cover a same/later shortage; a later surplus NEVER covers an earlier protected requirement. Transfer = `feasibleReallocationQty = MIN(receiverRemainingShortage, donorRemainingSurplus, timelyTransferableQty)` (§12/§32). **A donor is NEVER reduced below its own protected actionable requirement.**

**41.5A Timely-transfer authority for CURRENT FACTORY STOCK (F1-7N-FA-3A — USER-resolved 2026-08-20, Resolution A).** For CURRENT FACTORY STOCK in Phase-1 planning-only surplus netting, the timing authority is COMPLETE when both hold: (1) §32A tier eligibility permits the donor→receiver pair (`evaluateReallocationEligibility(...).eligible === true` — same Master SKU AND `donorRank ≤ receiverRank`, T1–T3 only); and (2) the donor has releasable current-factory-stock planning surplus at an eligible source. Because the stock is ALREADY physically present at the factory and this is analysis-only planning netting (NOT shipment execution), the orchestrator **MUST NOT** introduce route / carrier / lead-time capacity math. Therefore, **ONLY AFTER** `evaluateReallocationEligibility` returns `eligible = true`, freeze `timelyTransferableQty = donorRemainingReleasableSurplusQty` — an EXPLICIT canonical pass-through for CURRENT FACTORY STOCK. It is **NOT** Infinity, **NOT** a lead-time assumption, **NOT** shipment feasibility, **NOT** physical reservation, **NOT** physical transfer, **NOT** carrier capacity. Canonical feasible qty remains owned by `feasibleReallocationQty = MIN(receiverRemainingShortage, donorRemainingReleasableSurplus, timelyTransferableQty)` (§12/§32) — **not rewritten**. `calculationDate` is a REQUIRED explicit caller/runtime input (**never** derived from the clock) passed with each `requiredByDate` through the EXISTING `classifyRequiredByWindow` tier classifier (no second T1/T2/T3 classifier). **Scope limit (frozen):** this timely-transfer authority applies ONLY to CURRENT FACTORY STOCK + Phase-1 planning-only netting. It **MUST NOT** auto-apply to Ongoing PO, unfinished production, In-Transit, Shipment allocation, Overseas transfer, carrier planning, or physical cross-company reservation — those retain their own lifecycle/timing owners (§42 / `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.5 / Shipment). Cross-references the system-wide integer-allocation authority §43.

**41.6 Eligibility (injected facts, no hard-coding).** A receiver may consume another's releasable surplus only when existing canonical eligibility/scope facts permit that factory source to serve it — reuse `eligibleFactoryWarehouseIds`, warehouse master, factory identity config, destination/receiver scope, `allocationPriority`, `requiredByDate`, and the factory-operation policy config (62_ `KM_FACTORY_OPERATION_CONFIG`). **No `if (company/marketplace/TW …)` inside the generic allocator** — special policy (incl. TW) is injected eligibility/scope facts only.

**41.7 Net Order Need authority (CORRECTED — F1-7N-FA-3B-R0, 2026-08-20; see §44).** The **LIVE production monthly** final residual / Net Order Need owner is **`KMTPP.projectTimePhasedSupply`** (`residualOrderNeedQty = MAX(0, destinationGapQty − overseasCoveredQty − factoryCoveredQty)`; carton via `KMCALC.calculateSuggestedOrderQty`), persisted through `47_`→`24_`→`KMPW`. `KMCALC.sumRemainingShortages` is a pure §12/§32 primitive and the terminal of the **standalone** §41 KMFSR model/tests — it is **NOT** the live monthly terminal owner (superseding the prior wording that named it the "sole Net Order Need owner"). §41/KMFSR introduces **no** second live Net Order Need formula: when integrated (FA-3B3) KMFSR adjusts per-receiver **factory coverage** that feeds KMTPP; KMTPP stays the single live residual owner.

**41.8 Snapshot semantics (recommendation/audit, NOT ledger).** `net_order_need_snapshot` = post-reallocation recommendation fact; `reallocation_in_qty_snapshot` = planning qty covered by eligible donor surplus; `reallocation_out_qty_snapshot` = planning-attributed factory supply released from a donor and consumed by eligible receiver(s). These are recommendation/audit snapshots — **NOT inventory-ledger transactions**.

**41.9 Conservation invariants (freeze for FA-3A tests).** (1) Σ initial factory allocation ≤ physical factory supply; (2) `reallocatedOut ≤ releasableSurplus` per donor; (3) `reallocatedIn ≤ pre-transfer remaining shortage` per receiver; (4) no donor protected balance goes negative; (5) each transfer decrements donor surplus exactly once; (6) each transfer decrements receiver shortage exactly once; (7) `netOrderNeed = Σ remaining shortage after legal reallocation`; (8) no unit simultaneously donor-retained and transferred; (9) no transfer bypasses eligibility/destination rules; (10) deterministic input → deterministic output. Physical conservation stays per-company; planning netting never mutates a physical pool.

**41.10 Manual authority.** The recommendation is advisory. Even when Suggested Order = 0 because planning surplus covered the shortage, the user MAY manually enter an Order Qty. The engine MUST NOT disable the Order Qty input, overwrite a user-entered Order Qty, or force recommendation acceptance. User-approved `order_qty` remains the transaction authority.

**41.11 Non-goals (frozen).** No physical reservation/deduction/movement/borrowing; no ownership transfer; no shipment allocation; no PO binding; no second Net Order Need formula; no §40 re-derivation/re-ordering; no schema/API/UI/Apps-Script change in the freeze. Consolidation into `assets/specs` is a **candidate only** — deferred until FA-3H live acceptance.

---

## 42. Supply Lifecycle Count-Once + Ongoing-Order Site-Allocation Source Contract (F1-7N-FA-3A.1 freeze)

**Status:** CONTRACT FROZEN (documentation). The count-once machinery in **42.1 already EXISTS and is implemented**; this section freezes it as the system-wide invariant and adds the Ongoing-Order (unshipped PO) supply type as a distinct upstream source (**42.2**, runtime NOT IMPLEMENTED → FA-3B0). Authors no new formula.

**42.1 SUPPLY LIFECYCLE CONSERVATION (count-once) — system-wide invariant.** One physical unit may belong to exactly ONE active supply lifecycle bucket within a single planning run. Canonical buckets (owner `supply-planning-ledgers.js:buildSupplyLedger`): ACTIVE = `COMMITTED_PRODUCTION`, `APPROVED_SHIPPING_PLAN`, `SHIPPED_IN_TRANSIT`, `DELIVERED_NOT_RECEIVED`, `RECEIVED_NOT_REFLECTED`, `CURRENT_STOCK`; EXCLUDED (visible, contribute 0) = `DRAFT`, `CANCELLED_INVALID`, `CORRECTION_REVERSAL`. Count-once identity = `supplyLineageRef`; the same lineage in >1 `(poolKey,bucket)` fail-closes `SUPPLY_LINEAGE_CONFLICT` (pool → `BLOCKED_CONFLICT`, `effectiveSupplyQty=0`). The **load-bearing cross-feed guard** is the status→bucket map (`supply-planning-source-facts.js`): a shipped/partial_shipped PO and a completed shipping-plan map to `OMIT_TRANSFERRED` (the Shipment becomes the sole incoming owner), so shipped PO qty is NEVER counted as both Ongoing-Order and In-Transit/Factory. Net Order Need (§12) consumes only these mutually-exclusive effective supply facts — no unit twice.

**42.2 Ongoing-Order (unshipped PO) supply type.** Distinct from CURRENT FACTORY STOCK (§40/§41), IN-TRANSIT (Shipment/Qualified Incoming), and RECEIVED INVENTORY. Definition + site allocation are owned by `REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC.md` §3.5:
- **CORRECTED (F1-7N-FA-3B0-PRE, 2026-08-20 — supersedes the prior `ordered_qty − shipped_qty` wording):** now that PO Receive posts received units into `factory_stock.fac_current_stock` (§42.3), the Ongoing / not-yet-received committed qty = `MAX(0, ordered_qty − completed_qty)` — the still-in-production commitment. The prior `ordered_qty − shipped_qty` would DOUBLE-COUNT the completed-not-shipped units that are now physical Factory Stock. `completedNotShippedQty = MAX(0, completed_qty − shipped_qty)` is represented by **Factory Stock physical inventory (§40/§41)** — NOT a second Ongoing-Order planning bucket. `shippedQty = MIN(completed_qty, shipped_qty)` exits to `SHIPPED_IN_TRANSIT` (deducted from `factory_stock` at dispatch, §22 Confirm-and-Ship). Count-once partition: `ordered_qty = notYetReceivedCommittedQty + completedNotShippedQty + shippedQty`. `remaining_qty = MAX(0, completed_qty − shipped_qty)` remains the available-to-ship subset (Factory Stock portion).

**42.3 PO Receive → Factory Stock count-once handoff (F1-7N-FA-3B0-PRE — IMPLEMENTED, backend).** `handleReceivePurchaseOrderLines_` (13_) now performs ONE atomic (LockService), journal-rollback-protected transaction per accepted line: `completed_qty += recvQty`; `remaining_qty = MAX(0, completed − shipped)`; `factory_stock.fac_current_stock += recvQty` for the line's `supplier_warehouse_id + sku`; and appends one `factory_stock_movements` row (`movement_type='po_receipt'`, `related_entity_type='purchase_order_receipt'`, `related_entity_id=purchase_order_line_id`). Factory destination is validated (`is_active` AND `is_factory_warehouse`) → fail closed (`PO_RECEIVE_FACTORY_WAREHOUSE_UNRESOLVED`); never inferred from name/company/sku. The factory write reuses the SHARED core `factoryStockApplyDeltaTx_` (21_) — no second stock-mutation implementation. Optional `idempotency_key` dedupes retries via the existing movement lineage/note fields (no schema change). No ratio/rounding (whole-unit integer transfer, §43/§11). No site/marketplace allocation, no §40/§41 invocation, no reservation, no PO/shipment side effects. Factory↔Shipment count-once already holds: dispatch (§22) deducts `fac_current_stock` and reconciles `shipped_qty` atomically, so shipped units exit Factory Stock.

**42.4 Ongoing-Order site projection (F1-7N-FA-3B0 — PURE RUNTIME IMPLEMENTED, NOT CONNECTED).** Reserved pure module `assets/js/core/supply-planning-ongoing-order-projection.js` (namespace **KMOOP**, `KMOOP.projectOngoingOrderSupply(input)`). Projects ONLY `notYetReceivedCommittedQty = MAX(0, ordered_qty − completed_qty)` into per-site facts; `completedNotShippedQty` / `shippedLifecycleQty` are returned as lifecycle DIAGNOSTICS ONLY and are NEVER emitted as incoming supply (completed = physical Factory Stock §42.3; shipped = In-Transit §22). **A1** (`ordered_qty == requested_qty`): site shares = `request_order_line_sources.requested_qty ÷ Σ requested_qty` (immutable lineage — uses `requested_qty`, NEVER `approved_qty`), applied to `notYetReceivedCommittedQty` even under partial receipt (no physical site reservation exists; residual is retained, never guessed). **A2** (`ordered_qty != requested_qty`): shares = `siteFcBasis_i ÷ Σ SAME-COMPANY siteFcBasis`, where `siteFcBasis` = the canonical monthly Regular-FC basis (`KMPCX.forecastShareQty` = Σ Regular FC over M+1..M+4, Special Event excluded — the SAME basis §7/KMAF use for monthly), consumed as an INJECTED fact (assembly of existing facts, no new formula). Single-company PO line (re-proven FA-3B0): the committed qty belongs entirely to `purchase_order_lines.company`; A1/A2 NEVER cross company (a cross-company source row / other-company FC site = fail closed / excluded). Integer conversion = §43 FLOOR (`allocatedQty_i = FLOOR(available × ratio_i)`; `Σ ratio ≤ 1.0` else fail closed; residual retained; NO ROUND/CEIL/largest-remainder/+1). Count-once: `Σ siteAllocations + unallocatedResidualQty = notYetReceivedCommittedQty` (completed/shipped units never appear). NOT connected to Monthly/Gap/Weekly/Request-Order runtime; does NOT call KMFSR, allocate §40/§41 factory stock, compute Net Order Need, round cartons, mutate physical state, or persist. 55 unit assertions.
- Site allocation of the ongoing portion (single-company PO line): **A1** (`ordered_qty == requested_qty`) preserves immutable `request_order_line_sources.requested_qty` per site; **A2** (`ordered_qty != requested_qty`) preserves the company snapshot and splits the company qty across its own sites by within-company forecast share (`siteFC ÷ Σ_company siteFC`) — **no cross-company denominator, no equal-split, no name heuristic**.
- **FC-share owner (reconciled):** NOT the weekly §7/§24.5 `demandWeight` (weekly-only) and NOT the KMDA warehouse ratios (marketplace→warehouse grain, `50_`); basis = existing per-site canonical forecast demand, normalized within-company as an ASSEMBLY of existing facts (no new formula), reserved for FA-3B0.
- This contract supplies upstream planning facts only; it modifies no §40/§41 logic and no physical stock.

---

## 43. System-wide physical/committed supply integer-allocation rounding + conservation authority (F1-7N-FA-3A freeze — owner-of-record)

**Status:** CONTRACT FROZEN. Single owner-of-record for the integer-conversion + conservation rule applied WHENEVER a proportional/ratio allocation converts **PHYSICAL or COMMITTED** supply into integer receiver quantities. Cross-references §24.5–§24.7, §35, §40.11, §SC-1A. Applies to physical/committed supply **ONLY** — NOT to demand/forecast distribution (which keeps its own §2D/§29F–G owners). Prefer this section; other documents cross-reference it rather than restating a variant.

**43.1 Ratio ceiling.** Aggregate ratio must never exceed 100%: `Σ ratio_i ≤ 1.0`. A config contract requiring full distribution uses `Σ ratio_i = 1.0`; `Σ ratio_i > 1.0` **MUST fail closed** (never silently normalized).

**43.2 Ratio precision.** Ratio precision may remain decimal — the ratio itself is **NEVER** truncated before multiplication.

**43.3 Integer conversion = FLOOR.** Integer physical/committed-supply allocation uses `allocatedQty_i = FLOOR(availableQty × ratio_i)`.

**43.4 Forbidden roundings.** For physical/committed-supply allocation, **NEVER** use ROUND / CEIL / round-half-up / largest-remainder / remainder redistribution / hidden +1 / "give the leftover unit to the largest site" — unless a separately frozen rule explicitly overrides this section.

**43.5 Hard non-over-allocation invariant.** `Σ allocatedQty_i ≤ availableQty` — never exceed available physical/committed supply, even by ONE unit.

**43.6 Residual is valid.** `residualQty = availableQty − Σ allocatedQty_i` may remain unallocated. Leaving 1–2 pcs residual is safer than over-allocating (which manufactures artificial availability / shortage risk). **UNALLOCATED PHYSICAL RESIDUAL ≠ RECEIVER-ATTRIBUTED RELEASABLE SURPLUS** — only receiver-attributed surplus participates in §41 donor reallocation; global unallocated residual does NOT, unless a later canonical rule authorizes it.

**43.7 Count-once.** Per §42.1, one physical/committed unit is allocated at most ONCE per planning run.

**43.8 FA-3A conformance + audit.** FA-3A (`KMFSR.projectSurplusReallocation`) performs **NO** ratio→unit conversion: its initial allocation reuses §35/§40 `allocateFactoryDeterministic`, which is deterministic INTEGER FIFO (never ratio / ROUND / CEIL / largest-remainder), and its reallocation transfers whole units bounded by `feasibleReallocationQty`. FA-3A therefore adds no rounding code and proves conservation by regression guard (§41.9). **Audit result:** the only physical-supply largest-remainder path in the pure core is `allocateOverseasSharedPool` (`supply-planning-allocations.js`, §24.7 weighted largest-remainder). It is category-B (physical supply) and predates this authority, BUT it already satisfies §43.5 (`Σalloc ≤ pool`, enforced by `verifyConservation`) and is **NOT** on the FA-3A path. Reconciling its largest-remainder→FLOOR is **OUT OF SCOPE** for FA-3A (a separate future slice) — recorded as a bounded follow-up, not a blocker.

---

## 44. Monthly Supply Architecture Reconciliation (F1-7N-FA-3B-R0 freeze — owner-of-record; USER decision 2026-08-20)

**Status:** CONTRACT FROZEN (documentation). Resolves `MONTHLY_SUPPLY_ARCHITECTURE_CONFLICT`. Authors no new formula; it fixes ownership and repositions the FA-3A/FA-3B0 pure libraries. Live-runtime facts cited from `43_api_v1_gap_materialization.gs`, `42_api_v1_recommendation_workspace.gs`, `24_`/`47_`.

**44.1 Canonical LIVE monthly engine = Architecture A (frozen).** The production Monthly / Order-Planning recommendation engine is the Apps-Script gap-materialization + recommendation-workspace pipeline. Frozen owners (NOT replaced in Phase-1):
- **Initial factory allocation:** `KMMSA.allocateMarketplaceReceiverSupply` (43_:476, uncontended) + `KMAR.allocateFactoryCrossCompany` (43_:565, contended cross-company). **NOT** §40 `allocateFactoryDeterministic`.
- **Time-phased supply / residual order need:** `KMTPP.projectTimePhasedSupply` → `residualOrderNeedQty = MAX(0, destinationGapQty − overseasCoveredQty − factoryCoveredQty)` (42_:499–507).
- **Suggested Qty:** `KMCALC.calculateSuggestedOrderQty` (carton CEILING, 42_:512).
- **Persistence:** `47_`→`24_ rpoGenerateRecommendationDraftLockedResult_`→`KMPW` → `request_order_allocation_drafts` / `_lines`.
- **Timing owner:** KMTPP ETA-month→tier (42_:493–497). `classifyRequiredByWindow` is NOT the live monthly timing owner.

Architecture B (the pure `resolveMonthlyRecommendationFacts` + KMFSR + KMOOP + `sumRemainingShortages` chain) is **NOT** the production monthly engine and is NOT migrated to. It stays a validated pure model.

**44.2 KMFSR repositioned = pure §41 surplus-reallocation LIBRARY.** KMFSR is NOT a replacement monthly factory allocator. Live integration invokes §41 surplus reallocation AFTER KMMSA/KMAR have produced the initial source-attributed factory coverage. `KMFSR.projectSurplusReallocation` runs its OWN §40 initial allocation (its Phase 3) and is therefore the **standalone-model** entry point only. **Required adapter — IMPLEMENTED (FA-3B1, 2026-08-20):** `KMFSR.reallocatePreallocatedFactorySupply(input)` accepts PREALLOCATED per-receiver source-attributed factory coverage (`{demandKey, receiverKey?, requiredByDate, projectedRequirementQty, eligibleFactoryWarehouseIds, initialAllocationBySource:{warehouseId→qty}}` + `calculationDate` + optional `unusedFactorySupplyQty`) and runs ONLY the frozen §41 protect→releasable-surplus→eligibility→transfer core (shared `runReallocation`, KMFSR Phases 4–10) — **no second §40 allocation** (executable spy-proven), no §41 math change. Both public entry points share one §41 core; `projectSurplusReallocation` is unchanged (270 assertions green); adapter adds 99 assertions. Output: adjusted per-receiver factory coverage + `transferLedger`. KMTPP then consumes the adjusted `factoryCoveredQty`. Invariant: reallocation never increases total factory coverage beyond the physical pool (§43).

**44.3 KMOOP repositioned = pure §42 ongoing-order site-projection LIBRARY.** KMOOP produces integer site-attributed `notYetReceivedCommittedQty` (A1 request-source share; A2 same-company monthly FC share; §43 FLOOR + residual). It does **NOT** own monthly timing. Integration adapter **KMOTA IMPLEMENTED (F1-7N-FA-3B2, 2026-08-20)** — `supply-planning-ongoing-order-tpp-adapter.js`, `KMOTA.projectOngoingOrderIncoming({ ongoingProjection: <KMOOP output>, poLineTiming:[{purchaseOrderLineId, expectedCompletionDate}], tierByMonth })` → KMTPP incoming facts `{ availableDate, qty, incomingId: '<poLineId>||<receiverKey>', sourceType:'ONGOING_ORDER', tier }` (plus passthrough receiver identity KMTPP ignores). Pure, additive, no KMOOP/KMTPP internal change; `tier` set by the caller `tierByMonth` mechanism. 46 assertions (10 round cases + outside-horizon + fail-closed + resolver isolation + determinism/immutability + end-to-end KMTPP feed). **Ongoing timing authority — PRE AUDIT RESULT (F1-7N-FA-3B2-PRE, 2026-08-20; superseded by the R2 re-alignment below — the arrival-ETA requirement was dropped, not solved).** KMTPP incoming DTO (`supply-planning-time-phased-projection.js`) = `{ availableDate (REQUIRED strict YYYY-MM-DD; invalid → fail closed), qty, incomingId?, sourceType?, tier? }`; **KMTPP does not classify tier** — the caller (`42_`) sets `tier` from `availableDate` month via `tierByMonth` over the frozen KMPCX `M+1..M+4` window (a null tier still moves the running balance but appears in no monthly tier row). So a KMOOP→KMTPP adapter needs a per-site-allocation **`availableDate`** (destination-qualifying date). **Proven gap:** (1) the only ETA precedent is `shipments.eta` — a persisted/user-entered string carried VERBATIM (no owner computes ETA from source-date + lead-time; missing → review flag, never derived); (2) the lead-time owner `55_` returns a **SKU-scalar `leadTimeDays` only** (no origin/destination grain, no transit/processing split, no anchor date, no arrival date — cannot produce a destination-arrival ETA without new logic); (3) PO-line `expected_completion_date`/`expected_ship_date`/`inspection_date` are user-entered, copied-from-request, **often blank**, and factory-side (there is no `actual_completion_date` on the PO line). Therefore an ongoing-PO destination-arrival ETA cannot be produced today without either a semantic choice or invented transit math → `ONGOING_ORDER_MONTHLY_TIMING_AUTHORITY_MISSING` stands. **Do NOT invent lead-time/transit days; do NOT clock-fallback.**

**DECISION RESOLVED — `D-ONGOING-TIMING` (USER, F1-7N-FA-3B2 R2, 2026-08-20) = Option A, anchor = `expected_completion_date`.** The audit conflated two lifecycle concepts; the USER re-aligned the business semantics: a **destination-arrival ETA is NOT required for ongoing PO supply**. `ONGOING_ORDER` = **future factory supply** (`MAX(0, ordered − completed)`, not yet physical). Its planning availability date is *"the date this committed PO quantity becomes future factory-available"*, **NOT** an Amazon/marketplace/overseas destination arrival ETA. Canonical **PHASE-1** anchor:

> `ONGOING_ORDER.availableDate = purchase_order_lines.expected_completion_date` (VERBATIM; no added transit).

`expected_completion_date` = expected production-complete / factory-ready date (request `expected_ready_date` at PO creation, [13_:1803]; supplier-confirmed ready date at confirmation, [13_:2030]). **NOT** `expected_ship_date` (that is the later departure/shipment lifecycle), NOT clock, NOT `request_bucket`, NOT `55_` lead-time. Blank / non-strict-ISO → fail closed **`ONGOING_ORDER_ETA_UNRESOLVED`** (never counted as available; no fabricated date). `tier` derives from the `availableDate` month via the SAME `42_`/`tierByMonth` KMPCX `M+1..M+4` mechanism; a valid date outside the window → `tier = null` (KMTPP carries the balance, no invented T5+). `request_bucket` remains **provenance only** — never converted directly into an availability month.

**PHASE_1_BASELINE_TIMING_AUTHORITY (evolution point recorded).** The `expected_completion_date` rule is the Phase-1 baseline so the Monthly Order-Planning cycle can operate deterministically — it is **not** the permanent end-state logistics model. Phase-2+ may refine/supersede it with production-readiness / cargo-ready / sailing / booking / transit / destination-receiving authorities. Architectural requirement (held): the anchor is isolated behind ONE resolver (`KMOTA.resolveOngoingAvailableDate_`), so Phase-2 can replace the timing authority WITHOUT touching KMOOP quantity/site math, KMTPP time-phased math, KMFSR, lifecycle count-once, or persistence identity. Phase-2 transit/shipment timing is explicitly **not** designed or implemented in this slice.

**44.4 Live Net Order Need owner = KMTPP (single).** `KMCALC.sumRemainingShortages` is a pure primitive / standalone-§41-model terminal only — NOT the live monthly terminal owner (corrects §41.7 and prior FA-3A/FA-3B0 wording). Exactly one live monthly final-order-need owner: KMTPP residual. No second live Net Order Need formula is introduced.

**44.5 Lifecycle count-once (monthly, frozen).** Buckets: `DESTINATION_CURRENT`, `OVERSEAS_CURRENT`, `FACTORY_CURRENT`, `ONGOING_ORDER_NOT_RECEIVED`, `SHIPMENT_IN_TRANSIT`. One supply lineage → exactly one active bucket per run. Post-FA-3B0-PRE: completed PO qty = `FACTORY_CURRENT` only; not-yet-completed = `ONGOING_ORDER_NOT_RECEIVED` only (KMOOP); shipped = `SHIPMENT_IN_TRANSIT` only. KMOOP ongoing supply MUST NOT enter Factory Current or In-Transit.

**44.6 Intended live operation order (audited; KMTPP-composed).** 1) demand/gross gap (unchanged); 2) opening Site/Destination supply; 3) Overseas coverage; 4) KMMSA/KMAR initial factory allocation; 5) **§41 KMFSR surplus reallocation over the already-allocated coverage** (FA-3B1 adapter) → adjusted `factoryCoveredQty`; 6) **KMOOP ongoing-order site supply** → KMTPP incoming (FA-3B2, timing-gated); 7) shipment ETA incoming; 8) `KMTPP.projectTimePhasedSupply`; 9) `residualOrderNeedQty`; 10) carton CEILING; 11) persistence. Coverage terms 4–7 are composed as KMTPP opening/incoming inputs, not as a second residual formula.

**44.7 §43 preserved.** KMOOP ratio→unit = FLOOR + retained residual, no largest-remainder. KMFSR/Architecture-A factory coverage never exceeds the physical pool; no supply-side proportional allocation exceeds 100%.

**44.8 Persistence contract (audited).** `request_order_allocation_draft_lines` already has `net_order_need_snapshot`, `reallocation_in_qty_snapshot`, `reallocation_out_qty_snapshot`, `factory_available_qty_snapshot` (15_:41–43). **Gap:** no dedicated `ongoing_order_covered_qty` column (nearest: `qualified_incoming_snapshot`, `approved_supply_snapshot`). Prefer runtime/transport diagnostics for ongoing coverage first; no schema change forced in reconciliation. The live gap-backed writer currently populates only `calculated_gap_qty_snapshot`/carton fields (47_:194) — leaving the reallocation/factory snapshots blank; FA-3B4 validates populating them.

**44.9 Status + slices.** KMFSR / KMOOP status = **IMPLEMENTED_NOT_PRODUCTION_CONNECTED** until corrected FA-3B lands. Slices: **FA-3B1** = KMFSR preallocated-input §41 adapter — **IMPLEMENTED (`reallocatePreallocatedFactorySupply`, pure, not production-connected)**; **FA-3B2** = KMOOP→KMTPP incoming adapter (KMOTA) — **IMPLEMENTED (pure, not production-connected)**: `D-ONGOING-TIMING` resolved (Option A, PHASE-1 anchor `expected_completion_date`, see §44.3); no arrival-ETA / transit / shipment dependency; count-once and receiver lineage preserved; timing isolated behind `resolveOngoingAvailableDate_` for a Phase-2 swap; **FA-3B3a** = Architecture-A live Ongoing-PO wiring — **IMPLEMENTED (F1-7N-FA-3B3a, 2026-08-20)**. Single-authority chain KMSF→KMOOP→KMOTA→KMTPP wired into the live recommendation runtime (see §44.11); Ongoing PO only; Factory §41 (`D-FACTORY-KMFSR-INPUT`) deferred to **FA-3B3b**; **FA-3B4** = persistence/diagnostics validation. No second engine; KMMSA/KMAR/KMTPP never replaced.

**44.10 assets/specs consolidation manifest (deferred until FA-3H live acceptance).** Candidates carried for later promotion (NOT promoted now): Architecture-A monthly owner (KMMSA/KMAR + KMTPP), §41 KMFSR library integration, §42 KMOOP integration, KMTPP final residual owner, monthly lifecycle count-once (§44.5), and the §43 FLOOR rule. Planning MD remains the working SSOT.

**44.11 FA-3B3 live-integration audit result (F1-7N-FA-3B3, 2026-08-20; read-only; HALT — no runtime code changed).** The AUDIT-FIRST trace of the live Architecture-A monthly runtime (`42_api_v1_recommendation_workspace.gs`, `43_api_v1_gap_materialization.gs`, generated bundle `90_`) established, with exact evidence:

- **One KMTPP call site** (`42_:499`, inside `recoWsBuildMonthlyProjection_`). Two per-lane incoming lists feed it — marketplace `mIncoming` (`42_:612/625`) and warehouse `whIncoming` (`42_:729/732`) — **both built exclusively from shipment / qualified-incoming events** (KMSLS shipment-line candidates; KMPS `warehouseQualifiedEvents`). **No factory and no ongoing-PO events flow through any incoming list.**
- **Factory coverage enters as OPENING supply**, folded scalar (`recoWsComposeOpeningSupply_`, `42_:458-463`; consumed `42_:624`), NOT as incoming and NOT as a residual subtraction; the residual coverage terms are hardcoded `0` (`42_:506-507`), so `residualOrderNeedQty == destinationGapQty`. Produced by `gapOpBuildSupplyAllocation_` (`43_:443`) via `KMMSA.allocateMarketplaceReceiverSupply` (`43_:476`) + the frozen R2G-B contended cross-company pre-pass `KMAR.allocateFactoryCrossCompany` (`43_:565`), crossing 43_→42_ as the request field `supplyAllocationByReceiver` (`43_:681`). **No §41 surplus reallocation exists anywhere in the live path.**
- **The live runtime reads NO purchase-order data.** All input is one canonical snapshot read (`KMPS.readCanonicalSnapshots`, `42_:846`, `43_:255`) over a fixed 19-tab `CANONICAL_TABLES` registry (`90_:10955`) — **no `purchase_order_lines`, no `request_order_line_sources`.** Ongoing/committed supply is not represented today.
- **A different, canonical, designed-but-unwired Ongoing-PO seam already exists in Architecture-A:** the lifecycle bridge `KMSF.projectSupplyLifecycle` has a `committedProduction` input (`90_:5408`) whose `PRODUCTION_STATUS_MAP` (`90_:5192-5196`, sourced from REQUEST_ORDER_AND_PURCHASE_ORDER_SPEC §1) maps PO status `issued/in_production/partial_completed/completed → COMMITTED_PRODUCTION` (an active bucket, `90_:1373`), with `partial_shipped/shipped → OMIT_TRANSFERRED` (shipment becomes the incoming owner). It is deliberately not wired — "committedProduction — separate slices, not wired here (like the PO read)" (`90_:8218-8219`, `:8456-8457`). This is a **status-driven, whole-line lifecycle** model — categorically different from the FA-3B0/B1/B2 pure chain (KMOOP `MAX(0, ordered−completed)` + A1/A2 site share; KMOTA `expected_completion_date` timing).

**Why HALT (audit not clean):** wiring KMOOP/KMOTA as the live Ongoing-PO authority would (a) stand up a SECOND ongoing-PO supply path competing with the architecture's own intended `KMSF committedProduction` slot — violating the FA-3B3 constraints "no second engine / no parallel formulas / do not create Architecture C"; and (b) require registering NEW canonical tables (`purchase_order_lines` + `request_order_line_sources`) in the frozen generated bundle. And §41 live wiring requires reconstructing source-warehouse-attributed factory allocation (`initialAllocationBySource`, plus per-receiver `requiredByDate`/`projectedRequirementQty`/`eligibleFactoryWarehouseIds`) inside the frozen R2G-B conservation core — `gapOpBuildSupplyAllocation_` currently collapses factory coverage to a scalar `factoryCoveredQty` with no source grain (`43_:456/472/478`). Both are genuine authority/architecture decisions, USER-owned; the agent will not improvise them.

**DECISION 1 — `D-ONGOING-RUNTIME-AUTHORITY` (which mechanism owns live Ongoing-PO supply?).** *Option A (architecture-consistent):* wire the EXISTING seam — add the canonical PO read + feed `committedProduction` into `KMSF.projectSupplyLifecycle` (COMMITTED_PRODUCTION bucket). Reuses the proven count-once lifecycle bridge; but it is status/whole-line grain and delivers NO A1/A2 site attribution and NO `expected_completion_date` Phase-1 timing (KMOOP/KMOTA then remain reference models). *Option B:* make KMOOP→KMOTA the runtime owner — inject KMOTA incoming events into the marketplace incoming list and register the PO + `request_order_line_sources` canonical reads; delivers site attribution + Phase-1 timing, but must keep `committedProduction` dormant to preserve count-once (accepts a second ongoing path by contract). *Option C (hybrid):* KMSF `committedProduction` owns the count-once quantity; KMOOP/KMOTA refine it into site + Phase-1-timing attribution downstream — needs an explicit precedence contract. **Recommended: Option A** (uses the canonical seam; smallest new architecture), with site-attribution/timing (B/C) deferred to a later slice only if the business needs per-site ongoing granularity in the monthly cycle.

**DECISION 2 — `D-FACTORY-KMFSR-INPUT` (how to feed §41 in the live path?).** *Option A:* extend `gapOpBuildSupplyAllocation_` + the R2G-B pre-pass to retain per-source-warehouse factory allocation, assemble the KMFSR receiver set, apply `KMFSR.reallocatePreallocatedFactorySupply` over the full scope, and fold the adjusted per-receiver `factoryCoveredQty` back into opening supply (no stock mutation) — bounded but edits frozen conservation code. *Option B (recommended for now):* DEFER live §41 reallocation (keep factory coverage as-allocated) and scope FA-3B3 to Ongoing-PO only, making §41 live wiring its own later slice with its own regression budget. **These are independent — either decision can proceed without the other.**

**FA-3B3 status (audit):** `FA_3B3_IMPLEMENTED = NO`, `ARCHITECTURE_A_LIVE_INTEGRATION_READY = NO`. No runtime code, no Apps Script, no schema changed in this audit. Prerequisite libraries KMFSR/KMOOP/KMOTA remain IMPLEMENTED_NOT_PRODUCTION_CONNECTED and are unaffected.

**44.12 FA-3B3a — Ongoing-PO single-authority LIVE integration IMPLEMENTED (F1-7N-FA-3B3a, 2026-08-20).** `D-ONGOING-RUNTIME-AUTHORITY` RESOLVED by USER: **one chained runtime**, not two competing engines —

> RAW `purchase_order_lines` → **KMSF** (lifecycle / count-once admission authority; `committedProduction`→`COMMITTED_PRODUCTION`) → **KMOOP** (site-allocation authority; A1 request-source share / A2 same-company FC share) → **KMOTA** (Phase-1 future-factory availability-date authority) → **KMTPP** (the ONE live monthly time-phased consumer).

- **New pure owner:** `supply-planning-ongoing-order-runtime.js` (**KMOOR** `projectOngoingIncomingForSku`) chains the three libraries. KMOOP does NOT independently admit raw PO qty: KMSF admits the frozen `notYetReceivedCommittedQty = MAX(0, ordered_qty − completed_qty)` FIRST (under its existing COMMITTED_PRODUCTION / `in_production` vocabulary — no invented status), then KMOOP's allocatable total is **bounded to and cross-checked equal to** the KMSF-admitted qty per lineage (`ONGOING_ADMITTED_QTY_MISMATCH` fail-closed). Received (completed) and shipped (⊆ completed) units are excluded by the arithmetic → count-once vs Factory-Current opening supply holds.
- **Canonical reads (Analysis-Layer, read-only):** `purchase_order_lines` + `request_order_line_sources` added to `CANONICAL_TABLES` (`supply-planning-production-source.js`); no schema/table/column added; no PO write/mutation/reservation.
- **Live injection:** `42_` `recoWsExpandMarketplace_` calls `recoWsOngoingIncomingForReceiver_` (KMOOR) and concatenates the per-receiver `{incomingId, eta = expected_completion_date, qty, sourceType:'ONGOING_ORDER'}` events into the **SAME** `mIncoming` list `recoWsBuildMonthlyProjection_` already feeds to the single `KMTPP.projectTimePhasedSupply` call — no second KMTPP call, no second residual formula. Tier is derived by the existing `tierByMonth` from the `eta` month (M+1..M+4). Blank/invalid `expected_completion_date` → `ONGOING_ORDER_ETA_UNRESOLVED` → the lineage is admitted/traceable but contributes zero timed incoming.
- **Bundle:** KMOOP/KMOTA/KMOOR registered in `assets/tools/build-apps-script-bundle.js` (`MODULE_ORDER`+`GLOBALS`); `90_generated_supply_planning_bundle.gs` regenerated by the sanctioned tool (never hand-edited) → 49 modules.
- **Factory §41 untouched** (`D-FACTORY-KMFSR-INPUT` deferred to FA-3B3b): `gapOpBuildSupplyAllocation_`/KMMSA/KMAR/`supplyAllocationByReceiver` factory semantics unchanged; KMFSR not wired here.
- **Tests:** `supply-planning-ongoing-order-runtime-f1-7n-fa-3b3a.test.js` (28 assertions, CASES A–L). Full regression 0 new failures. `APPS_SCRIPT_SYNC_REQUIRED = YES` (42_ + regenerated 90_); `GENERATED_BUNDLE_REBUILD_REQUIRED = YES` (already rebuilt).

**44.13 FA-3B3b — Factory source-grain preservation + §41 LIVE reallocation IMPLEMENTED (F1-7N-FA-3B3b, 2026-08-20).** `D-FACTORY-KMFSR-INPUT` RESOLVED by USER: keep the EXISTING Architecture-A initial factory allocation (`KMMSA.allocateMarketplaceReceiverSupply` + R2G-B `KMAR.allocateFactoryCrossCompany`) as the ONLY initial allocation; preserve its **source-warehouse grain**; feed `KMFSR.reallocatePreallocatedFactorySupply`.

- **Source grain preserved non-destructively (no rerun, no guess):** the per-source-warehouse allocation is already produced by `KMALLOC.allocateFactoryDeterministic` (`allocations[].sourceWarehouseId`), passed through KMAR verbatim and exposed by KMMSA as `res.factory.allocations`. `gapOpBuildSupplyAllocation_` now additively records `factoryBySource: {warehouseId: qty}` per receiver alongside the unchanged scalar `factoryCoveredQty` (multi-receiver: group the FIFO draws; single-receiver: the eligible pools); the R2G-B pre-pass `gapOpComputeFactoryContention_` additively records `partitionBySource` (grouping KMAR's `allocations` by `demandKey` + original `sourceWarehouseId` — the **physical** source survives cross-company, never receiver/company/synthetic). Invariant per receiver: `Σ factoryBySource == factoryCoveredQty`. **The initial-allocation math / R2G-B conservation is unchanged (additive only).**
- **§41 live (`gapOpApplyFactorySurplusReallocation_`, new):** runs `KMFSR.reallocatePreallocatedFactorySupply` per **(company, masterSku)** over the source-attributed coverage — **NO second §40 allocation** (spy-proven: 0 `allocateFactoryDeterministic` calls on this path), exactly ONE initial allocation remains. `projectedRequirementQty` = the PASS-1 `calculatedGap` (demand − site stock, the existing KMTPP-owned residual) — **NOT a second Net Order Need formula**. `unusedFactorySupplyQty = 0` (§43.6: unused physical pool is never donor surplus). Non-actionable **T4 / out-of-window** receivers are excluded (visibility-only; coverage untouched). Post-reallocation coverage = `initial − reallocatedOut + reallocatedIn` replaces `factoryCoveredQty` in the receiver map, which 42_ folds into KMTPP opening supply — **Architecture-A KMTPP remains the single Net Order Need owner; no second KMTPP call, no second residual**. Fail-safe: KMFSR/KMCALC unbundled or a group throws → that group's coverage unchanged.
- **PLANNING-ONLY:** no `factory_stock` mutation, no reservation, no physical movement, no PO/Shipment/Request-Order change. `PHYSICAL_CROSS_COMPANY_RESERVATION_DEFERRED` stays true. Conservation: `Σ coverage == physical pool` (no supply created); `Σin == Σout`; per-source `Σ ≤ pool`; coverage ≥ 0.
- **Diagnostics (Phase 8, runtime DTO only — persistence is FA-3B4):** `allocMap[rk]` carries `factoryAvailableQtySnapshot` / `reallocationInQty` / `reallocationOutQty`; 42_ projects them onto `mLine.factorySurplusReallocation`. No schema change.
- **Bundle:** KMFSR registered in `build-apps-script-bundle.js`; `90_` regenerated by the sanctioned tool → 50 modules, `bundle_sha256=ff766a0a1ff4b367b155c4b170fd83db0690230243632002a770bf36462125e7`.
- **Tests:** `factory-source-grain-reallocation-f1-7n-fa-3b3b.test.js` (28 assertions, CASES A–M against the real bundled allocators). Full regression 0 new failures. `APPS_SCRIPT_SYNC_REQUIRED = YES` (42_ + 43_ + regenerated 90_). **FA-3B4** (persistence) is the next slice.

**44.14 FA-3B4 — §41 diagnostic persistence AUDIT → HALT (F1-7N-FA-3B4, 2026-08-20; read-only; no runtime code changed).** Objective: persist the three FA-3B3b §41 snapshots into the EXISTING `request_order_allocation_draft_lines` columns `factory_available_qty_snapshot` / `reallocation_in_qty_snapshot` / `reallocation_out_qty_snapshot` (transport-only, no recompute, no schema change). The full production (gap-backed) chain was traced:
- **Producer (present):** `43_ gapOpApplyFactorySurplusReallocation_` stamps `factoryAvailableQtySnapshot`/`reallocationInQty`/`reallocationOutQty` on the alloc entries (`43_:545-546`); on re-projection `42_ recoWsExpandMarketplace_` carries them to `mLine.factorySurplusReallocation` (`42_:665-666`).
- **DROP point:** `43_ gapOpMapFromLines_` (`43_:137-168`) maps the re-projected lines into the materialized `order_planning_gap` row using ONLY the `monthlyProjection` tiers; it never reads `L.factorySurplusReallocation`. The `order_planning_gap` schema `OP_GAP_HEADERS_` (`43_:36-39`) has **no** column for the three values, so they are discarded here.
- **Downstream (would transport if present):** `47_ recGenMapGapRowToFacts_` builds the persisted `snapshotRow` from `gapRow.tN_*` only (`47_:190-200`); `24_ rpoResolveFacts_` passes `body.facts.lines` verbatim (`24_:47`); `KMPW`→`KMPB projectLine` (`extraRow`/`detail.row`)→`KMPPB`→`KMPR applyLineOps` copy every `snapshotRow` key to its matching DB column by NAME. The three target columns exist in the draft-line schema (`15_:41-43`) and in KMPW's frozen schema — downstream transport is name-based and would carry the values with zero edits **if they reached `47_`**.
- **Why HALT (condition 4 — schema change required):** the ONLY production persistence path reads the materialized `order_planning_gap` sheet, which drops the values and has no slot. Carrying them requires adding 3 columns to `OP_GAP_HEADERS_` AND migrating the live `order_planning_gap` sheet — and `prodRequireSheet_(ss, order_planning_gap, OP_GAP_HEADERS_)` (`43_:795`) validates the sheet **fail-closed** via `classifySchemaMismatch` (`29_:57-63`), so an unmigrated sheet would break the whole gap batch. That is a schema change the FA-3B4 mandate ("existing-schema only; do not add columns") forbids and an agent must not perform unilaterally. Token: **`FACTORY_REALLOCATION_DIAGNOSTIC_TRANSPORT_REQUIRES_GAP_TABLE_SCHEMA`**. No recomputation is involved — the values are already produced at 43_; only a transport slot is missing.

**DECISION REQUIRED — `D-B4-GAP-TRANSPORT-SCHEMA` (one USER choice unblocks FA-3B4):** authorize the minimal additive schema change to the INTERNAL `order_planning_gap` materialization table (NOT the canonical persistence schema, which is unchanged)? *Option A (recommended, invention-free):* add 3 additive columns to `order_planning_gap` (`factory_available_qty_snapshot`, `reallocation_in_qty_snapshot`, `reallocation_out_qty_snapshot`) + migrate the live sheet; then FA-3B4 is a pure two-hop field copy — `43_ gapOpMapFromLines_` copies `L.factorySurplusReallocation.*` into the gap row, `47_ snapshotRow` reads the new gap-row columns, and the existing name-based downstream writes the (already-existing) draft-line columns. No recompute, no formula, no final-schema change. *Option B:* leave §41 diagnostics runtime-only (`mLine.factorySurplusReallocation`, exposed FA-3B3b) and do not persist them. Net Order Need / Suggested Qty are unaffected either way.

**Ongoing-PO diagnostic persistence (Phase 6 read-only audit):** `ONGOING_PO_DIAGNOSTIC_PERSISTENCE_GAP` — `request_order_allocation_draft_lines` (`15_:37-48`) has NO ongoing-PO / committed-production coverage column (nearest existing: `qualified_incoming_snapshot`, `approved_supply_snapshot`, both shipment-incoming semantics). Recorded as a bounded follow-up; NOT a blocker and NOT solved here (no column added).

**FA-3B4 status (audit):** blocked on `D-B4-GAP-TRANSPORT-SCHEMA` — **RESOLVED below (§44.15, Option A approved, IMPLEMENTED F1-7N-FA-3B4-R1).**

**44.15 FA-3B4-R1 — §41 diagnostic persistence transport IMPLEMENTED (F1-7N-FA-3B4-R1, 2026-08-20).** `D-B4-GAP-TRANSPORT-SCHEMA = Option A` (USER-approved): 3 additive columns on the INTERNAL `order_planning_gap` materialization transport the FA-3B3b §41 facts into the ALREADY-EXISTING `request_order_allocation_draft_lines` columns. Transport-only — no recompute, no formula, final persistence schema unchanged.
- **Producer (unchanged):** `43_ gapOpApplyFactorySurplusReallocation_` → `42_ mLine.factorySurplusReallocation` (FA-3B3b).
- **Schema (additive):** `OP_GAP_HEADERS_` (`43_`) gains `factory_available_qty_snapshot` / `reallocation_in_qty_snapshot` / `reallocation_out_qty_snapshot` at the END (constant `OP_GAP_FACTORY_SNAPSHOT_COLS_`); readers are name-based (order-agnostic). `order_planning_gap` transports — it does NOT own their calculation.
- **43_ transport:** `gapOpMapFromLines_` copies `L.factorySurplusReallocation.{factoryAvailableQtySnapshot,reallocationInQtySnapshot,reallocationOutQtySnapshot}` VERBATIM into the gap row (numeric incl 0 preserved; absent/null stays MISSING → blank cell — never a fabricated 0). No KMFSR/allocator call.
- **47_ transport:** `recGenMapGapRowToFacts_` copies the gap-row columns onto the PRIMARY (T1) actionable-tier draft `snapshotRow` ONLY (receiver-level fact; never replicated across T1/T2/T3 → no Σ inflation); 0 preserved, blank stays blank.
- **Downstream (unchanged, name-based):** `24_ rpoResolveFacts_` (`body.facts.lines`) → KMPW → KMPB `projectLine` (`snapshotRow`→`detail.row`; the 3 columns are NOT in `LIVE_ANALYSIS_FORBIDDEN`) → KMPPB → KMPR `applyLineOps` writes every key to its matching header → the 3 EXISTING `request_order_allocation_draft_lines` columns (`15_:41-43`). No writer change, no new writer.
- **Conservation:** `Σ reallocation_in == Σ reallocation_out` per §41 scope — equality carried from upstream runtime truth; persistence performs NO rounding/CEILING/FLOOR/ratio/balancing. KMTPP remains the single Net Order Need owner; KMCALC the Suggested-Qty/carton owner (both unaffected). No physical mutation, no reservation.
- **Ongoing PO:** `ONGOING_PO_DIAGNOSTIC_PERSISTENCE_GAP` unchanged (no column added; not overloaded) — deferred.
- **Migration (live sheet) — USER-run via the sanctioned owner:** `prodMigrateAppendColumns_` (`29_production_safety_adapter.gs`) — auth-gated (`KMSAFE.validateMigrationAuthorization`), idempotent, additive-append (rerun → 0 added; no duplicate/reorder/data-loss). NOT wrapped in any handler (governance: migration-only twins are reachable only from the authorized tool, INIT.4). **`MIGRATION_BEFORE_NEW_EXEC_REQUIRED = YES`** — `prodRequireSheet_` validates `OP_GAP_HEADERS_` fail-closed, so the sheet MUST carry the 3 columns before the FA-3B4-R1 runtime runs a gap batch.

  **Release ordering (USER-owned):** (1) push; (2) sync Apps Script sources `43_api_v1_gap_materialization.gs` + `47_api_v1_recommendation_generation.gs`; (3) **before running any gap batch**, run the one-time migration from the Apps Script editor — `prodMigrateAppendColumns_(SpreadsheetApp.openById(<OPERATION_DB_ID>).getSheetByName('order_planning_gap'), OP_GAP_FACTORY_SNAPSHOT_COLS_, <migrationAuthDTO>)` where the DTO carries `{migrationId, expectedSpreadsheetId, expectedSheetName:'order_planning_gap', expectedOldHeaderHash: KMSAFE.headerHash(<21 pre-migration headers>), expectedNewHeaderHash: KMSAFE.headerHash(OP_GAP_HEADERS_), backupReference, execute:true, actor}`; (4) verify the 3 headers exist exactly once; (5) create a new Apps Script deployment version; (6) smoke-test: gap generation → §41 snapshots in `order_planning_gap` → recommendation draft → values in `request_order_allocation_draft_lines`. No bundle change (90_ unchanged; no core-JS edited).

**44.16 FA-3C-PRE1 — contended-Factory partition MULTI-PROPERTY storage (F1-7N-FA-3C-PRE1, 2026-08-20).** STORAGE-TRANSPORT ONLY: the ONE canonical R2G-B contended-Factory partition (`{contendedSkus, partition, partitionBySource}` — FA-3B3b `partitionBySource` included) is now persisted across multiple Script Properties instead of inline in the single job-state value. The logical partition, R2G-B/KMMSA/KMAR allocation, per-source conservation, §41, KMTPP Net Order Need, and Suggested Qty are ALL unchanged. Root cause: the single job-state property (`GAP_JOB_ORDER_PLANNING`) embedded the partition; at 228 contended receivers the serialized partition reached 8319 B > the conservative safe per-value budget `GAP_JOB_PARTITION_SAFE_BYTES_ = 6912` (Script Properties ~9216 B/value), tripping the truthful terminal `CONTENDED_FACTORY_PARTITION_SIZE_LIMIT` (46_). USER authorized the multi-property design.
- **Protocol (46_, standalone Apps Script — NOT bundled):** `gapJobPartitionWrite_` chunks `JSON.stringify(contention)` into UTF-8-safe pieces each `<= GAP_JOB_PARTITION_SAFE_BYTES_` (never splitting a surrogate pair), writes them under generation-specific keys `GAP_JOB_ORDER_PLANNING:PART:<runId>-<rev>:<NNNN>`, and returns META `{version, generationId, chunkCount, byteLength, checksum (FNV-1a)}` stored in job state. `gapJobPartitionRead_` reconstructs by reading exactly `chunkCount` chunks, verifying byte-length + checksum, then `JSON.parse` — **fail-closed** (`CONTENDED_FACTORY_PARTITION_CHUNK_MISSING` / `_CHECKSUM_MISMATCH`), never a partial or mixed generation. **Atomic generation:** chunks written first, META persisted LAST by the existing `gapJobWriteState_`, then `gapJobPartitionCleanup_` prunes prior generations (prefix-bounded); `gapJobClearState_` calls `gapJobPartitionPurge_` (prefix-bounded) on job clear/done. **Backward-compatible:** a v1 inline `state.factoryContention` still reads verbatim. `state.factoryPartitionRev` gives crash-safe monotonic generations (an unpersisted rev is simply reused, overwriting the same keys).
- **Kept:** the `GAP_JOB_PARTITION_SAFE_BYTES_` per-chunk budget is unchanged (never raised); the single-value size guard is superseded — an oversize partition is now chunked and the pre-pass ADVANCES to COMPANY_SLICES (never auto-splits allocation, never falls back to per-company double-use). Total Script-Properties quota (~500 KB) is comfortably safe: 228 receivers ≈ 8319 B (~2 chunks); even 10× ≈ 83 KB (~13 chunks) — no `CONTENDED_FACTORY_PARTITION_TOTAL_PROPERTY_QUOTA_RISK`.
- **Tests:** `contended-factory-partition-storage-f1-7n-fa-3c-pre1.test.js` (22 assertions, CASES A–L: round-trip, 228-receiver multi-chunk, each chunk ≤ safe, key-order-independent, missing/corrupt fail-closed, v1 backward-compat, generation swap, prefix-bounded cleanup, determinism) + R2G-B `factory-contention-prepass` §9/M4 updated to the new authorized behavior (48/0). Full regression 0 new failures. `APPS_SCRIPT_SYNC_REQUIRED = YES` (`46_api_v1_gap_materialization_job.gs` only); no bundle change (90_ unchanged); no schema change; no formula change.

**44.17 FA-3C-PRE2 / PRE3 — AI Plan draft transport + observability (F1-7N-FA-3C-PRE2..PRE3-R2, 2026-08-20/21).** OBSERVABILITY + TRANSPORT ONLY — NO recommendation math, Suggested Qty, order_qty, carton, gap, §41, or persistence-gate change. Three defects on the Order Planning → AI Support → **AI Plan** path (canonical resumable request-order-draft job: `48_` + router + `request-order.js`; per-SKU authority stays the EXISTING R4E2 locked persister — recommended_qty verbatim from `order_planning_gap`):
- **PRE2-R2 (CORS transport):** the four `requestOrderDraft.job.{start,continue,status,cancel}` router dispatches returned a RAW `gapBatchEnvelope_` object (the `46_` gap-job convention) but were NOT wrapped in `jsonResponse_`; `doPost` then returned a non-`ContentService` HTML page with no `Access-Control-Allow-Origin`, CORS-rejecting the browser fetch → `HTTP_TRANSPORT_ERROR`. Fixed by wrapping the four dispatches in `jsonResponse_(...)` in `01_router.gs`, mirroring the working `orderPlanningGap.job.start`. `APPS_SCRIPT_SYNC_REQUIRED = YES` (`01_router.gs`); no bundle/schema/math change.
- **PRE3-R1 (terminal observability):** the frontend DONE path discarded the job's canonical terminal `counts` and always claimed success. Fixed to thread `counts` and build a truthful message via pure `_roAiPlanDoneMsg_` (success = created+reused+regenerated; NEVER a blanket success at 0 created). Frontend-only.
- **PRE3-R2 (reason distribution + persistent panel):** (a) `48_` now folds each non-success SKU's canonical reason `code` into a **bounded** diagnostic histogram `reasonCounts` + capped `reasonSamples` (distinct codes capped → `OTHER`; ≤5 SKU samples/code; codes truncated to 48 chars), stored in the SAME Script-Property job state and surfaced on the public envelope — `APPS_SCRIPT_SYNC_REQUIRED = YES` (`48_api_v1_request_order_draft_job.gs`); (b) `request-order.js` renders a PERSISTENT "AI Plan Result" panel (counts + reason distribution) that survives the getActive re-render and hides on a scope change (cleared on a new run / dismiss) — FRONTEND_DEPLOY_REQUIRED = YES. No bundle change (90_ unchanged); no schema change.
- **STILL OPEN (data-gated, not code):** the zero-persisted-draft ROOT CAUSE for ResUS/US/Amazon (99 eligible → 0 drafts) is **not yet proven** — the coarse buckets refute empty-scope/gap-not-READY/scope-mismatch (99 entered + reached terminal); the leading candidate is `UNITS_PER_CARTON_UNAVAILABLE` (missing `sku_details.units_per_carton`), which the new `reasonCounts` will pin on the next live run. Whether missing carton data should legitimately BLOCK a draft (vs. be diagnostic-only with a blank carton) is an **open authority decision**, deferred pending the live reason distribution — no defaults manufactured. **[SUPERSEDED by PRE3-R3 below: `notReady=0`; the two live reason families are both schema-related.]**

**44.18 FA-3C-PRE3-R3 — AI Plan persistence schema gate is TYPE-SCOPED; request-order draft-lines live-sheet migration (F1-7N-FA-3C-PRE3-R3, 2026-08-21).** Live retest was decisive: `Processed=99, Created=0, Failed=99`, `notReady=0` (UNITS_PER_CARTON REFUTED), split into two schema families — `RECOMMENDATION_SCHEMA_NOT_READY: shipping_alloca…` = 4 and `PRODUCTION_SAFETY:HEADER_MISSING [request_order_…]` = 95. The 4/95 split is exactly the `skipSchemaValidation` boundary (validate the authorized schemas ONCE per continuation → ~4 first-SKUs hit the read-only gate; the other ~95 skip it and hit the write-path validator).
- **DURABLE RULE (code fix):** the pre-write authorized-schema gate (`KMPW.assertAuthorizedSchemasReady` → `validateAuthorizedRecommendationSchemas` → `authorizedTableSpecs`) is now **scoped to the `recommendationType` being generated** — it validates ONLY that type's own header+lines table + the shared run journal (`recommendation_calculation_runs`), the exact set the write path writes (`[cfg.header, cfg.lines, RUN_JOURNAL]`). A `MONTHLY_ORDER` request-order draft never reads or writes the `WEEKLY_SHIPPING` `shipping_allocation_*` tables, so a stale/absent shipping-allocation schema must NOT hard-gate it. An absent/unknown type still validates all authorized tables (backward-compatible). This narrows VALIDATION SCOPE only — no header contract changes, and the fail-closed guard is UNCHANGED for every table a type actually writes (a stale `request_order_allocation_draft_lines` still blocks `MONTHLY_ORDER`). Owner: `supply-planning-production-writer.js` (`authorizedTableSpecs(recommendationType)`); caller `24_recommendation_orchestrator.gs` `rpoValidateSchema_(ss, type)`. Bundle 90_ rebuilt.
- **USER MIGRATION (not code — the primary persistence blocker):** the 95-SKU family is a **live-sheet-vs-SSOT gap** — the live `request_order_allocation_draft_lines` sheet is missing one or more canonical columns from the SSOT `REQUEST_ORDER_ALLOCATION_DRAFT_LINES_HEADERS_` (`15_request_allocation_handlers.gs`). Since the S0 production-safety round made `procurementEnsureSheet_`/`sheetEnsureColumns_` **validate-only** (no auto-append), the sanctioned fix is the authorized migration twin `prodMigrateAppendColumns_` run by the USER; the runtime deliberately never provisions/repairs a live sheet. No defaults manufactured, no guard weakened.

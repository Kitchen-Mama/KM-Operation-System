...
>   - `unshippedOngoingQty = MAX(0, ordered_qty − shipped_qty)` — the still-committed future supply that REMAINS in Ongoing-Order authority. **This is NOT `remaining_qty`.** `remaining_qty = MAX(0, completed_qty − shipped_qty)` is only the produced-but-unshipped (available-to-ship) SUBSET; `unshippedOngoingQty` also includes the not-yet-produced `MAX(0, ordered_qty − completed_qty)`. Timing of the ongoing portion follows existing rules (`request_bucket` T1/T2/T3 provenance retained per PO line + `expected_completion_date`/`expected_ship_date`); no duplicate tier logic.
>

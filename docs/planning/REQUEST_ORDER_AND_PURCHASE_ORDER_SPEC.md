...
> ### §3.5 Ongoing Order site allocation + supply-lifecycle partition (F1-7N-FA-3A.1 freeze; runtime landed F1-7N-FA-3B0)
> **Runtime status (F1-7N-FA-3B0, 2026-08-20):** this contract is now implemented PURE + NOT CONNECTED by `assets/js/core/supply-planning-ongoing-order-projection.js` (`KMOOP.projectOngoingOrderSupply`) — A1 immutable request-source share, A2 within-company monthly `KMPCX.forecastShareQty` share, §43 FLOOR + residual, count-once (`MAX(0, ordered − completed)`; completed→Factory Stock / shipped→In-Transit excluded). Not wired to Monthly/Gap/Weekly/Request-Order runtime. See `SUPPLY_PLANNING_CALCULATION_RULES.md` §42.4.
> **Scope:** upstream planning FACTS only for the Ongoing-Order (unshipped PO) supply type. This contract does NOT mutate any PO/company snapshot, reserves/moves nothing physical, and supplies inputs to Net Order Need (§SUPPLY_PLANNING §12/§14) — it does not alter that formula. It is a SEPARATE supply type from CURRENT FACTORY STOCK (§40/§41), IN-TRANSIT (Shipment/Qualified Incoming), and RECEIVED INVENTORY (physical inventory owner).
>
> **(1) Company commitment is a transactional snapshot (unchanged).** `purchase_order_lines.km_qty/resus_qty/restw_qty` are the per-company user decision captured at PO creation (each PO line is single-company by construction — exactly one non-zero; §3.4). **"No ratio allocation" applies at the COMPANY grain** and is unchanged — the commitment layer never recomputes the company split. The site allocation below is a NEW, LOWER (within-company) planning grain and does not touch the company snapshot.
>
> **(2) Lifecycle partition of a PO line (derives from existing quantities — NO new field, "new/old order" is conversational only):**
>   - `shippedQty = shipped_qty` — has crossed the PO→Shipment boundary; **exits Ongoing-Order authority** (owned by `shipment_line_allocations` / `SHIPPED_IN_TRANSIT`; the PO no longer counts it, §26/§28). It does **NOT** automatically become `factory_stock` — the destination/execution state owns the next bucket. `related_shipment_id` is a declared-but-UNUSED column; the real PO↔Shipment link is `shipment_line_allocations.purchase_order_line_id`.
>   - **CORRECTED (F1-7N-FA-3B0-PRE, 2026-08-20 — supersedes `ordered_qty − shipped_qty`):** since PO Receive now posts received units into `factory_stock.fac_current_stock` (SUPPLY_PLANNING §42.3), the Ongoing / not-yet-received committed qty = `notYetReceivedCommittedQty = MAX(0, ordered_qty − completed_qty)` — the still-in-production commitment only. The produced portion `completedNotShippedQty = MAX(0, completed_qty − shipped_qty)` has BECOME physical **Factory Stock** (§40/§41), NOT a second Ongoing bucket — counting `ordered − shipped` here would double-count it. `remaining_qty = MAX(0, completed_qty − shipped_qty)` is exactly that Factory-Stock (available-to-ship) subset. Count-once: `ordered_qty = notYetReceivedCommittedQty + completedNotShippedQty + shippedQty`. Timing of the ongoing portion follows existing rules (`request_bucket` T1/T2/T3 provenance retained per PO line + `expected_completion_date`/`expected_ship_date`); no duplicate tier logic.
>
> **(3) Site allocation of the unshipped Ongoing portion — two cases (per single-company PO line):**
>   - **CASE A1 — `ordered_qty == requested_qty`** (both persisted on the PO line; test needs no live RO read): the original recommendation was accepted in full, so preserve the original site provenance — use `request_order_line_sources.requested_qty` (per `company+country+marketplace` site; that column is append-only IMMUTABLE) as the site basis. **Do NOT rescale by FC.** Invariant: `Σ source.requested_qty = request_order_line.requested_qty = ordered_qty`.
>   - **CASE A2 — `ordered_qty != requested_qty`**: the original source quantities are no longer the exact commitment. Preserve the (single) company qty snapshot, then allocate that company's committed qty across THAT company's eligible sites by the canonical within-company forecast share — basis = each site's own canonical monthly forecast demand (the existing per-site Regular FC / KMPD planning demand), normalized within the company: `siteQty = companyCommittedQty × (siteFC ÷ Σ_company siteFC)`. **No cross-company FC denominator; no equal-split; no marketplace-name heuristic** (all forbidden — the monthly path fail-closes `DEMAND_ALLOCATION_RULE_NOT_CONFIGURED` / `DEMAND_WEIGHT_UNRESOLVED` rather than fake a share). If a company has one eligible site it gets 100%. Conservation: `Σ_company siteQty = that company's committed PO qty`.
>   - **FC-share OWNER (reconciled):** this within-company site share is **NOT** the WEEKLY §7/§24.5 `demandWeight` (weekly-only; `buildMonthly` emits none) and **NOT** the KMDA configured warehouse ratios (`50_`, a different marketplace→warehouse grain). Its basis is the existing canonical per-site forecast demand; the company→site normalizer is an ASSEMBLY of existing facts (no new formula) reserved for FA-3B0 — see SUPPLY_PLANNING §42.
>
> **(4) PO source-lineage snapshot risk (flagged, no schema now):** CASE A1 reads `request_order_line_sources` AFTER PO creation via the retained `request_order_line_id`. The `requested_qty` column is append-only immutable (safe), but `approved_qty` on source rows CAN be re-synced by later RO edits (`syncLineSourceApproved_`). A1 therefore binds to source `requested_qty` (immutable) ONLY. A future PO-creation snapshot of the site split would fully harden this — deferred, no schema added this round.
...
> **⚠️ SUPERSEDED / `request_order_allocation_draft_lines` = `DEPRECATED_PENDING_MIGRATION` (F1-7N-FA-3C-DRAFT-MODEL-R1, 2026-08-21).** The MONTHLY_ORDER decision workspace is being **FLATTENED** to a single SKU-level `request_order_allocation_drafts` row with fixed `t1_/t2_/t3_` decision + provenance columns; the child-line table is retired after migration + runtime cutover. The exact frozen V2 schema, migration/cutover/test contracts, and ID/timestamp/status rules are canonical in [`REQUEST_ORDER_ALLOCATION_DRAFT_V2_FLATTEN_DESIGN_FREEZE.md`](./REQUEST_ORDER_ALLOCATION_DRAFT_V2_FLATTEN_DESIGN_FREEZE.md). No live-DB/schema/runtime change yet (`DB_CHANGE_READY = NO`). Formal `request_orders`/`request_order_lines`/`request_order_line_sources` and PO models are **unchanged**. The §3.7 tables below describe the still-live pre-flatten model.

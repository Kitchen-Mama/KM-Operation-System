...
**Field mapping (added):**
- **`company`** = the site owner (KM / ResUS / ResTW …) this line's demand belongs to. **Each line maps to ONE company** — the KM/ResUS/ResTW split shown in the Draft UI is the **set of per-company lines** for a `(sku, bucket)`. There is **no separate company-split table** yet (see §12.13 — `request_order_line_sources` is spec-only).
- **`inspection_date` / `expected_ready_date` / `expected_ship_date`** — decision-layer schedule fields, entered per tier in the Draft and written to **every line of that tier** on Save (blank until entered).
- **`request_bucket`** = `T1` / `T2` / `T3` — the demand bucket, **preserved on every line** and **never merged at the Request stage**. `request_month` = `YYYY-MM` the bucket maps to (T1 = next month … T3 = +3).
- **`requested_qty`** = original requested qty from the Order Allocation draft. **`approved_qty`** = editable approval qty in the Request Order Draft. **`final_order_qty`** = locked qty at Submit/Approve (= `approved_qty` for now); cleared on Reject.
- **`forecast_qty`** ← draft `fc_qty_snapshot` (bucket-month FC in the current mapping). **`current_stock`** ← draft `site_stock_snapshot`.
- **`on_the_way_qty`** / **`factory_allocated_qty`** / **`shortage_qty`** / **`reallocation_qty`** — blank in Phase 1 (no shipment-overview join / allocation engine / shortage formula / reallocation engine yet).
- **`calculation_method`** = source label (`manual_order_allocation` from Send Request; `system_suggestion` / `placeholder` reserved). **`line_status`** = `draft` / `submitted` / `approved` / `cancelled`. **`linked_purchase_order_line_id`** = blank until a PO line is created (traceability lives in `request_order_po_links`, future).

> All added columns are **additive** — `sheetEnsureColumns_` appends any missing header to existing tabs; no existing column is altered. `estimated_amount` on the header remains the Total Amount source of truth.

**Primary vs deprecated columns (mapping finalization).** Columns kept as **primary source of truth**: `company`, `request_bucket`, `request_month`, `series`, `requested_qty`, `approved_qty`, `shortage_qty`, `carton_qty`, `units_per_carton`, `inspection_date`, `expected_ready_date`, `expected_ship_date`, `calculation_method`, `line_status` (+ reserved denormalized `km_qty` / `resus_qty` / `restw_qty` / `recommended_qty` — see note). The following are **DEPRECATED / no longer primary** (still physically present for back-compat; **not deleted** from live sheets, **not** relied on as source of truth): `final_order_qty`, `forecast_qty`, `current_stock`, `on_the_way_qty`, `factory_allocated_qty`, `reallocation_qty`, `source_company_count`, `source_site_count`, `product_name`, `need_reason`, `related_entity_type`, `related_entity_id`. Company/site/month allocation detail is owned by **`request_order_line_sources`** (§3.8); the per-line `company` column carries the primary company. `km_qty` / `resus_qty` / `restw_qty` are **reserved denormalized snapshots** — currently the KM/ResUS/ResTW split is derived from the per-company lines, not stored in these columns.
...
- **✕ (cancel tier):** soft cancel — sets `request_order_lines.line_status = 'cancelled'` for the tier's lines (kept in DB, block hidden). If **no active line remains** on the request, `request_orders.status = 'cancelled'` + `cancelled_by/at`. Handler `cancelRequestOrderTier`. *(request_order_line_sources status is not updated — spec-only table; follow-up.)*
...
- **Company-split storage (missing structured field — documented, no silent behavior):** the KM/ResUS/ResTW split is represented as **one `request_order_line` per company** (via the new `company` column) — there is **no** `request_order_line_sources` company-split column implemented. Consequences: (a) re-allocating Approved to a company that has **no existing line** for that `(sku, bucket)` is **not supported** in Phase 1 (would require creating a new company line = out of scope); (b) `request_order_line_sources` (append-only source audit incl. company allocation) remains **spec-only**. Both are follow-ups.
...
**Status (this task):**
- **Read path implemented:** `03_master_data_handlers.gs` `validTabs` includes `request_order_line_sources`; adapter `KM.DB.getRequestOrderLineSources()` + `normalizeRequestOrderLineSourceRecord` (exposes `tierType`, `sourceMonth`; `requestedQty`/`approvedQty`/`shortageQty` as numbers). Returns `[]` when the tab is absent.
- **Write path PENDING (not implemented):** no handler populates this tab yet, so the Company Allocation popup **falls back** to `request_order_lines` grouped by company and shows **"Site-level source pending."** `tier_type` / `source_month` are added at the **spec** level; when a write handler is built it should `sheetEnsureColumns_` these columns.

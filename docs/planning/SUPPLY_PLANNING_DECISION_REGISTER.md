...
### PART B — CROSS-RUN / CROSS-COMPANY FACTORY CONSERVATION = **HALT (NOT FROZEN)**
...
- **HALT token: `FACTORY_CROSS_RUN_CONSERVATION_REQUIRES_RESERVATION_AUTHORITY` (unless cross-company batch is authorized).** **Smallest next decision (user-owned):**
...
- **FACTORY_CROSS_RUN_CONSERVATION_AUTHORITY = NOT_FROZEN.** OVERSEAS is unaffected (company-owned FBA/3PL; per-company conservation is physically correct).

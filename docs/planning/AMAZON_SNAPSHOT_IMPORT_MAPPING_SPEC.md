...
- **Idempotent + failure-safe:** guarded by a `LockService` script lock (two triggers can't prune/upsert at once); a failed BigQuery read aborts **before** any prune/write (valid retained data is never destroyed); running twice the same day yields identical data and no duplicate keys.
  - **Date-key representation defect fixed in source (HOTFIX A, 2026-08-03):** the natural key previously compared the **incoming** `snapshot_date` (normalized to `yyyy-MM-dd`) against the **existing** Sheet cell via a raw `String(cell)`. When the destination cell was a Google-Sheets **Date value**, `String(Date)` produced a locale/timezone string that never matched `yyyy-MM-dd`, so `rolling_upsert` **appended instead of updated** and each run silently added a duplicate row (coverage/prune shared the same defect via `String(cell).slice(0,10)`). Fix: `keyOfRowArr`/`keyOfObj` (09), `amazonReadDestDateCoverage_` (08), and the retention prune now canonicalize the `snapshot_date` key part through the single `amazonNormalizeDate_` normalizer, so a **Date value and a `yyyy-MM-dd` string for the same calendar day yield a byte-identical key**. Verified by `assets/tests/amazon-daily-sales-date-key.test.js` (Date-vs-string UPDATE-not-APPEND, mixed-representation duplicate collapse last-wins, repeated-run idempotency, coverage present-not-missing, retention parity). **Natural key, BigQuery grain, triggers, and the 90-day policy are unchanged.** Historical duplicate cleanup remains a **separate pending task** (not executed here); **not yet deployed**; **live-Sheet confirmation of the `snapshot_date` cell type still pending** (§ live-verification note).
  - **Historical duplicate dry-run tooling + migration contract (HOTFIX A2, 2026-08-03):** the source fix stops *future* duplicate appends but does **not** remove rows already present. A **pure, read-only** dry-run analyzer — `assets/js/core/amazon-daily-sales-dedup.js` (`analyzeAmazonDailySalesDuplicates` / `buildAmazonDailySalesDedupPlan`; tests `assets/tests/amazon-daily-sales-dedup.test.js`, 23 assertions) — was landed to assess and stage the one-time cleanup. It reuses the **identical** natural key and date-normalization semantics as the importer (byte-compatibility asserted against the extracted `amazonNormalizeDate_`), classifies every duplicate group as `IDENTICAL_FACTS` / `METADATA_ONLY_DIFFERENCE` / `CONFLICTING_FACTS` / `INVALID_KEY`, and proposes a **last-wins (later-row-order)** keep/remove plan. **No live-deletion function exists in this module.** Eligibility: `IDENTICAL_FACTS` + `METADATA_ONLY_DIFFERENCE` → `AUTO_ELIGIBLE`; `CONFLICTING_FACTS` → `REVIEW_REQUIRED` (never auto-removed); missing/invalid key → `BLOCKED`. **Actual production duplicate counts are UNKNOWN pending a live read or a Sheet export** (this environment has no Sheet connector) — see the export request below.
    - **Future one-time migration contract (NOT executed):** `DRY_RUN` default; execution requires an explicit `execute:true` flag **plus** a matching expected pre-migration row count, expected duplicate-extra-row count, and dry-run report hash. Ordered steps: acquire `ScriptLock` → read full data region → validate headers → create an immutable timestamped backup (duplicate Sheet tab **and/or** CSV/XLSX export, never Sheets version-history alone) → compute normalized keys → classify → **HALT if any `REVIEW_REQUIRED`/`BLOCKED` group exists unless explicitly authorized** → retain the deterministic last-wins winner for `AUTO_ELIGIBLE` groups only → preserve unrelated rows + unknown columns → write only after count confirmation → verify post-write row count → rerun the duplicate scan (auto-eligible extras = 0) → verify sample keys → record a migration-audit row → release lock in `finally`. Abort on any count/header mismatch, new conflict, or backup failure; **no partial cleanup, no automatic retry, no externally reachable destructive router action.** Rollback = restore from backup → validate row count → rerun duplicate report → verify key samples → record reason. **Migration remains unexecuted; no rows changed; no deployment.**
    - **Export request (to obtain the counts):** provide a CSV/XLSX of `amazon_daily_sales_snapshot` (full data region, header row included) containing the natural-key fields (`snapshot_date, country, marketplace, channel, sku`), all canonical sales measures (`currency, sales_units, sales_amount, sales_amount_usd, return_units, total_orders, session, page_view, unit_session_percentage, buy_box_percentage, browser_session, browser_page_views, app_session, app_page_view`), and the metadata (`source_row_hash, sync_batch_id, created_at, updated_at, synced_at`). Preserve original cell types where possible (a Date-typed `snapshot_date` is itself diagnostic). The analyzer then emits `amazon_daily_sales_duplicate_summary.json` / `_groups.csv` / `_dry_run.md` as dry-run evidence.
    - **Real-data dry-run result (HOTFIX A2-DATA, 2026-08-03):** analyzed the user-provided export `Operation System Database - amazon_daily_sales_snapshot.csv` (input SHA-256 `a822affc5d2744a299a3425d4c33a1b0b000d993e2b893daea577f4a24c808a6`; analyzer commit `2c30c359`; **report hash `e859defe5b7d56146d09f021f966aca9212f86231954c2fc18f750e64d520a7e`**, deterministic, reproduced 3×). **12,882 rows → 6,500 unique keys; 6,382 duplicate groups; 6,382 extra rows; max 2 rows/key.** Classification: **0 IDENTICAL_FACTS, 6,139 METADATA_ONLY_DIFFERENCE (AUTO_ELIGIBLE), 243 CONFLICTING_FACTS (REVIEW_REQUIRED), 0 INVALID_KEY, 0 BLOCKED.** Projected output after auto-eligible cleanup only = **6,743** rows (12,882 − 6,139; balance verified). Affected range **2026-06-03 → 2026-07-30** (58 days, 9 countries, 96 SKUs, channel=Amazon). **All 6,382 groups carry an identical `source_row_hash` → production data confirms the repeated-append mechanism.** The 243 CONFLICTING groups differ **only** in two derived display fields (`buy_box_percentage` `$100.00` vs `100`; `unit_session_percentage` `33` vs `33.33`) — **no core sales measure conflicts** — and are held for manual review (conservative string compare; never auto-merged). CSV flattened all cells to text so original cell-type is not preserved (5 keys show mixed `M/D/YYYY` vs `yyyy-MM-dd` raw formats, consistent with the Date-vs-string mechanism). **Downstream:** the Inventory Replenishment 7-day Sales Trend (SUM by date, no dedup) is **doubled (+100%)** for affected SKUs on affected dates (measured CO1100-R/US 2030 vs 1015); Avg Sales/Day, Days-of-Supply, Order/Site need, and Shipping-Plan Avg fallback read the *weekly* snapshot and are **unaffected**; the §22 normalized engine fails-fast on duplicates and has no live feed yet. **Migration readiness = `READY_WITH_MANUAL_REVIEW`** (243 REVIEW_REQUIRED groups need an explicit user decision before cleanup; 6,139 AUTO_ELIGIBLE separable safely). Evidence artifacts are dry-run-only, held outside Git (contain full-row data); **migration remains unexecuted; no live rows changed; HOTFIX A deployment still pending; backup + rollback required before any execution.**
    - **Execution-prep + authorized classification refinement (HOTFIX A3-PREP, 2026-08-03):** the user **authorized** last-wins cleanup of the 243 groups that differ only in `buy_box_percentage` representation/formatting and `unit_session_percentage` precision (rationale: identical `source_row_hash`, identical core sales measures, later row also carries the latest `updated_at`/`synced_at`). The analyzer gained an **opt-in** `tolerantDerivedFields` refinement (default OFF, so the conservative A2 result is unchanged): for **only** those two derived fields, equality is by numeric meaning (strip a leading currency char + trailing `%`, parse finite number) — never for core measures, and a non-numeric derived value still stays a real conflict. Tests: `assets/tests/amazon-daily-sales-dedup.test.js` now 29 assertions (adds T1–T5 covering the refinement, core-measure protection, non-numeric fallback, and a guard that a caller cannot force a core field tolerant). **Provisional final dry-run WITH the refinement on the existing export** (input SHA-256 `a822affc…`; refinement `buy_box_percentage`,`unit_session_percentage`; **provisional report hash `fb5f89930a3049541772d399a2810bac9fe8959f243c11d6f7136ec21c45f989`**, reproduced 3×): 12,882 rows → **0 IDENTICAL, 6,382 METADATA_ONLY (of which 243 authorized-derived-format), 0 CONFLICTING, 0 INVALID → 6,382 AUTO_ELIGIBLE, 0 REVIEW_REQUIRED, 0 BLOCKED**; projected output = **6,500** (= unique keys; balance verified). **This dry-run is PROVISIONAL** — it ran against the earlier export, not a fresh post-deployment export. **Deployment, the production smoke test, the trigger pause, the immutable backup, and the fresh export could NOT be performed in this environment (no Apps Script/Sheets/clasp/gcloud access)** — they are user-manual Google-UI steps. **Execution readiness = `BLOCKED`** for now, gated solely on those five user-side actions (classification itself is clean). The authoritative A3 export hash + report hash MUST be recomputed on the fresh post-deployment export before any execution. HOTFIX A source (`0f777ef`) is verified present/committed but **NOT yet deployed to the Apps Script project**; **no live rows changed; no historical cleanup executed.**

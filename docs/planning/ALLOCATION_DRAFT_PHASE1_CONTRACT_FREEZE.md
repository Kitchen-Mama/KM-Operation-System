...
- **A read that did not answer** — **added 2026-09-22 by S2-R3.** `NO_ACTIVE_DRAFT` and “the readback did not answer” are **two different facts and now have two different states.** An unanswered initial load / Refresh is **`DB_UNKNOWN`**, never `SAVE_FAILED` (nothing was being saved) and never `NOT_SAVED` (the database said nothing of the kind); it carries the transport code, clears any previously loaded draft rather than leaving it on screen, and reports `source: 'LOCAL_UNVERIFIED'` when a sessionStorage buffer is what the operator is looking at — distinct from the `source: 'LOCAL'` that a genuine `NO_ACTIVE_DRAFT` produces. While that stands the page discloses it and **Submit Plan is refused** with `EXECUTION_PLAN_DB_STATE_UNKNOWN`, because a durable shipping plan may not be built on a picture nothing has confirmed — including a draft another session has since submitted or cancelled. It clears itself: the next read that answers drops it, whatever the answer is. Nothing is written, discarded or merged.

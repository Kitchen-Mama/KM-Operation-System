...
| Column | Note |
|---|---|
| `event_fc_id` | **PK** (target name; the live handler currently uses `event_id` — see reconciliation note) |
...
**Reconciliation — PENDING (do not assume written):** the live `14_fc_write_handlers.gs` header (§3) uses **`event_id`** as PK and does **not** yet include `event_fc_id / campaign_id / campaign_sku_line_id / marketplace_id / fc_share / source / status`. The Special Event modal's Save **passes** `source='manual_fc_summary'`, `status='active'`, and blank `campaign_id / campaign_sku_line_id`, but the header-based writer **silently drops** columns absent from the sheet header, so those fields are **not persisted yet** (the `sku / event_name / event_period / year / fc_qty / company / country / marketplace / category / series` columns that DO exist in the header are written). Aligning the handler header (add `campaign_id / campaign_sku_line_id / source / status / marketplace_id / fc_share`, PK rename `event_id → event_fc_id`) is **pending Apps Script work** (out of this task's file scope — the task explicitly excludes Campaign write / `.gs` schema changes).

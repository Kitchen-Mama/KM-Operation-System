...
- **`campaign_id` is REQUIRED** (SOURCE-VERIFIED: `handleUpsertFcSpecialEvent_` rejects a body lacking `campaign_id` — `14_fc_write_handlers.gs:272-273`, error "Missing campaign_id — an event forecast must link to a campaign"). An event forecast must link to a campaign; orphan rows are structurally prevented.
...
- `handleUpsertFcSpecialEvent_` **rejects** an event lacking `campaign_id` (write campaign → line first), so orphan `fc_special_events` rows are structurally prevented.

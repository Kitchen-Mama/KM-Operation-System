...
> **AMENDMENT (F1-7N-FA-4B2, 2026-08-24) — runtime physical grouping key.** The canonical Submit runtime (`shippingPlanRouteGroupKey_`, 11_) uses the **full physical shipment-compatibility key**: `company + country + source_warehouse_id + ship_from + destination_warehouse_id + destination + shipping_method + **last_mile_delivery** + **planning_cycle**`. This extends the five-value key by making the source/destination **warehouse ids** explicit endpoint identities and by adding **last_mile_delivery** and **planning_cycle** so physically-incompatible lines never combine (different last-mile / planning-cycle → separate plans). **Marketplace stays excluded** (→ MULTI header, real marketplace per line). **Carrier is DEFERRED** — not authoritative at plan creation, so it is NOT a grouping dimension. Every dimension is bound into `spfp-1` (route fields incl. last_mile via the header fingerprint; planning_cycle via each line's `source_reason` `cyc:`).
...
- **Each line preserves its real line-level Marketplace + Site SKU.** LOGICAL field = `marketplace`; **the PHYSICAL live column is `shipping_plan_lines.marketplace_seperate`** (misspelled, deployed) — **CORRECTED (F1-7N-FA-4B1, 2026-08-24)** by the USER-frozen authority + the captured live-header diagnostic (the earlier "cleanly-spelled `marketplace`" claim here was stale/not-live-verified). Physical `site_sku` is unchanged. A single canonical accessor bridges logical↔physical; the line's marketplace always holds the **real** value (never `MULTI`); **no rename, no migration** (marketplace_seperate is the frozen physical DB contract). See `TWO_MAIN_FLOWS_CONNECTIVITY_AUDIT_F1-7N-FA-4A.md` (F1-7N-FA-4B1).
...
- **Physical consolidation across separate plans is an Execution-Layer concern**, handled by `shipment_plan_links` **after** the Shipment exists (§12.3) — never by merging Decision-Layer plan rows. **RUNTIME STATUS (F1-7N-FA-4B2):** `shipment_plan_links` is **SPEC_DEFINED_NOT_IMPLEMENTED** (DB/spec columns exist; NO runtime writer/reader). `shipment_lines.shipping_plan_line_id` is **ONE_SOURCE_ONLY** — exact for a 1:1 transfer but insufficient to represent multiple plan-line contributions (or their quantities) to one consolidated shipment_line; `shipment_line_allocations` is **PO_LINE_SUPPLY_ONLY** (a different axis). A consolidated `shipment_line_id + shipping_plan_line_id + contributed_qty` authority is **MISSING_SHIPPING_PLAN_LINE_CONTRIBUTION_AUTHORITY** — a required decision for the Shipping Plan approval → Shipment Draft → consolidated Shipment Lines batch. See `TWO_MAIN_FLOWS_CONNECTIVITY_AUDIT_F1-7N-FA-4A.md` (F1-7N-FA-4B2).

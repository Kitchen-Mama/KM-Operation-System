...
**Related:** [`SKU_DETAILS_LOGISTICS_SPEC.md`](./SKU_DETAILS_LOGISTICS_SPEC.md), [`DATABASE_RELATIONSHIP_MAP.md`](./DATABASE_RELATIONSHIP_MAP.md) §3, [`SUPPLY_CHAIN_ARCHITECTURE_PRINCIPLES.md`](./SUPPLY_CHAIN_ARCHITECTURE_PRINCIPLES.md) (Immutable Flow)
...
| **Logistics** | item / package / carton dimensions + weights + `units_per_carton`, `hscode` | v1 |
| **Pricing** | `declared_value`, `minimum_price`, `msrp`, `selling_price` (+ their units) | v1 |
...
| **Attributes** | product attributes / features | **Future** |
| **Images** | multiple images / assets | **Future** |

> The **Marketplace** tab surfaces the existing `marketplace_skus` / `pricing_list` relationship for the SKU; it does not redefine those tables. Supplier/Cost, Attributes, and Images are **future extension** tabs (empty placeholders in v1).
...
**Logistics — customs:** `hscode`, `declared_value`, `declared_value_unit`
**Pricing:** `minimum_price`, `minimum_price_unit`, `msrp`, `msrp_unit`, `selling_price`, `selling_unit`
...
- **Prices and declared value must be ≥ 0** (`declared_value`, `minimum_price`, `msrp`, `selling_price`).
- **Currency / unit fields must not be empty** (every `*_unit`).
...
**Dropdown fields:** `category`, `series`, `lifecycle`, `gs1_type`, `item_dimension_unit`, `item_weight_unit`, `package_dimension_unit`, `package_weight_unit`, `carton_dimension_unit`, `carton_weight_unit`, `declared_value_unit`, `minimum_price_unit`, `msrp_unit`, `selling_unit`, `pm`.
...
**weight_unit:** `kg`, `lb`
**currency_unit** (for `*_price_unit` / `declared_value_unit` / `selling_unit`): `USD`, `TWD`, `RMB`, `EUR`, `GBP`, `JPY`, `AUD`, `CAD`

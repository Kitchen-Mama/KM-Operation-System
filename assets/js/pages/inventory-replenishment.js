...
function submitReplenishmentPlans() {
    ...
    if (canCloudWrite) {
        // Stable execution key for THIS Submit intention (idempotency) — reused on retry, never regenerated by a
        // re-render/navigation, dropped only on a confirmed Decision Commit (_clearAllocationDraft).
        ...
        window.KM.DB.createShippingPlansBatch({
            ...
            submit_batch_id: submitExecutionKey,
            lines: planLines
        }).then(function(result) {
            if (result && result.success === false) {

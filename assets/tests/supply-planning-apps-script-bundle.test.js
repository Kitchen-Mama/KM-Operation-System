...
(function () {
  ...
  eq(b1.manifest.length, 20, 'A: manifest lists all 20 canonical modules');
...
(function () {
  ...
  ['KMCALC', 'KMQI', 'KMLEDGER', 'KMALLOC', 'KMLINE', 'KMPC', 'KMPR', 'KMPL', 'KMPB', 'KMPPB', 'KMORCH', 'KMUE',
   'KMSF', 'KMBRIDGE', 'KMSR', 'KMSI', 'KMSRP'].forEach(function (ns) {
  ...
  eq(ctx.KM_BUNDLE_INFO.modules.length, 20, 'C: KM_BUNDLE_INFO manifest present in runtime');
})();

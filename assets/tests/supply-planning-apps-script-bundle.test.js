...
(function () {
  ...
  eq(b1.manifest.length, 24, 'A: manifest lists all 24 canonical modules');
...
(function () {
  ...
  ['KMCALC', 'KMQI', 'KMLEDGER', 'KMALLOC', 'KMLINE', 'KMPC', 'KMPR', 'KMPL', 'KMPB', 'KMPPB', 'KMORCH', 'KMUE',
   'KMSF', 'KMBRIDGE', 'KMSR', 'KMSI', 'KMSRP', 'KMSP', 'KMPS', 'KMPW', 'KMVD'].forEach(function (ns) {
  ...
  eq(ctx.KM_BUNDLE_INFO.modules.length, 24, 'C: KM_BUNDLE_INFO manifest present in runtime');
})();

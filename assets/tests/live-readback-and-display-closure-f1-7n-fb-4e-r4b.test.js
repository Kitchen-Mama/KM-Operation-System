...
checks.push(Promise.resolve().then(function () {
  ...
  eq(cn.level, 'ZH_HANT_HOUSE_DISPLAY', 'E3 ... from the house display authority, reported as a level');
  eq(tw.level, 'ZH_HANT_HOUSE_DISPLAY', 'E3 ... for both');
  ...
  ['US', 'JP', 'GB', 'DE', 'VN', 'TH', 'MY', 'KR'].forEach(function (c) {
    eq(R.country(c).level, 'ZH_HANT_PINNED_SOURCE', 'E7 ' + c + ' still resolves from the vendored source');
  });
  var houseTable = /var HOUSE_COUNTRY_ZH = \{[\s\S]*?\};/.exec(resolverSrc);
  ok(!!houseTable, 'E8 the house table exists');
  eq((houseTable[0].match(/^\s{8}[A-Z]{2}:/gm) || []).length, 2, 'E8 ... and contains exactly TWO entries');
  ...
  ok(/feature\/map-texture-3/.test(resolverSrc), 'E11 the map-branch reconciliation note is recorded at the change');
  ok(/regenerat/i.test(resolverSrc), 'E11 ... and says what happens when the asset is regenerated');
}));

  ...
  return Promise.resolve()
    ...
    .then(function () {
      ...
      ok(!/function setSkuImageOverride\b/.test(OVR) && !/window\.setSkuImageOverride/.test(OVR),
        'E1a the image-override WRITER is absent from the source, not just from this list');
      ok(!/getSkuImageOverride\(/.test(extractFn(OVR, 'getNormalizedSkuImage'))
        ...
        'E1b and neither image function reads a browser override — sku_details.image_url is the owner');
      ok(d.sha === IMAGE_SHA, 'E2  the image surface matches the sealed digest', d.sha);
      ok(d.bytes === 2930, 'E3  and its byte count', d.bytes);
    })

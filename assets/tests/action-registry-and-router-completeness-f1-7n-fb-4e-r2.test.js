...
// FC-SUMMARY-R2B-A2-R1 — the migration tool joins the owned set. It is not runtime: no router action reaches
// it, the Apps Script Run dropdown is the only way in, and it is removed once the migration is accepted.
GS_OWNED_SINCE_R1['TEMP_migrate_fc_target_rules_header_r2ba2.gs'] = 'FC-SUMMARY-R2B-A2-R1 the USER-run, UNROUTED additive header migration for fc_target_rules: the live sheet is missing three REQUIRED columns (scope_type, scope_id, target_percentage), which is why Target Rule saves were refused with HEADER_MISSING rather than HEADER_ORDER_MISMATCH. It appends only those three at the right edge under a migration authorization DTO with verified header hashes, moves no existing column, touches no data row, and is idempotent: a second run is a NO_OP';

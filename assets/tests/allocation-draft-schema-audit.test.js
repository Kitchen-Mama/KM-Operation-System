...
ok(DRAFTS_CANON.length === 30 && LINES_CANON.length === 30, 'T4c canonical LIVE schema = 30-col header / 30-col line (R6F1: source_warehouse_id/code at the live position; accidental R3C2 source_allocated_qty_snapshot removed)');
ok(LINES_CANON.indexOf('source_allocated_qty_snapshot') < 0, 'T4c-2 accidental 31st field source_allocated_qty_snapshot is REMOVED (runtime authority == live 30-col schema)');
ok(LINES_CANON.indexOf('selected_source_warehouse_id') < 0 && LINES_CANON.indexOf('selected_shipping_method') < 0 && LINES_CANON.indexOf('user_edited') < 0, 'T4d NO selected_*/user_edited on the line');
